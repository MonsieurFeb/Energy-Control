...
public class CrossBigReactors extends CrossModBase {
    ...
    public List<FluidInfo> getAllTanks(TileEntity te) {
        ...
        if (te instanceof TileEntityReactorPartBase) {
            ...
            if (reactor.isPassivelyCooled()) return null;
            return FluidInfo.toFluidInfoList(
    ...
    public NBTTagCompound getCardData(TileEntity te) {
        ...
        if (te instanceof TileEntityReactorPartBase) {
            ...
            if (reactor.isPassivelyCooled()) tag.setDouble(DataHelper.OUTPUT, reactor.getEnergyGeneratedLastTick());
            else tag.setDouble(DataHelper.OUTPUTMB, reactor.getEnergyGeneratedLastTick());
    ...
    public int getHeat(World world, int x, int y, int z) {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (te instanceof TileEntityReactorPartBase) {
                MultiblockReactor reactor = ((TileEntityReactorPartBase) te).getReactorController();
                if (reactor != null) return (int) reactor.getReactorHeat();
            }
        }
        return -1;
    }
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_BIG_REACTORS,
            new Object[] { "RF", "PB", 'P', Items.paper, 'R', "dustRedstone", 'F',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'B', "ingotYellorium" });
