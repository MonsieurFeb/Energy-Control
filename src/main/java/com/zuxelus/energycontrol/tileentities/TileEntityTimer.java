...
public class TileEntityTimer extends TileEntityFacing implements ITilePacketHandler {
    ...
    public String getTimeString() {
        if (isTicks) return Integer.toString(time);
        int seconds = time / 20;
    ...
    public void setTime(int value) {
        int old = time;
        time = value;
        if (!worldObj.isRemote && time != old) notifyBlockUpdate();
    }
    ...
    public void setIsWorking(boolean value) {
        ...
        isWorking = value;
        if (isWorking) startingTime = time;
        if (!worldObj.isRemote && isWorking != old) notifyBlockUpdate();
    }
    ...
    public void setIsTicks(boolean value) {
        boolean old = isTicks;
        isTicks = value;
        if (!worldObj.isRemote && isTicks != old) notifyBlockUpdate();
    }
    ...
    public void onNeighborChange() { // server
        boolean newPowered = worldObj.getIndirectPowerLevelTo(
    ...
    public void onServerMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                if (tag.hasKey("value")) setTime(tag.getInteger("value"));
                break;
            case 2:
                if (tag.hasKey("value")) setInvertRedstone(tag.getInteger("value") == 1);
                break;
            case 3:
                if (tag.hasKey("value")) setIsWorking(tag.getInteger("value") == 1);
                break;
            case 4:
                if (tag.hasKey("value")) setIsTicks(tag.getInteger("value") == 1);
                break;
        }
    }
    ...
    public void onClientMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                if (tag.hasKey("value")) time = tag.getInteger("value");
                break;
            case 2:
                if (tag.hasKey("value")) isWorking = tag.getInteger("value") == 1;
                break;
        }
    }
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("timer")) time = tag.getInteger("timer");
        if (tag.hasKey("startingTime")) startingTime = tag.getInteger("startingTime");
        if (tag.hasKey("invert")) invertRedstone = tag.getBoolean("invert");
        if (tag.hasKey("isWorking")) isWorking = tag.getBoolean("isWorking");
        if (tag.hasKey("isTicks")) isTicks = tag.getBoolean("isTicks");
        if (tag.hasKey("poweredBlock")) sendSignal = tag.getBoolean("poweredBlock");
        if (tag.hasKey("isPowered")) isPowered = tag.getBoolean("isPowered");
    }
    ...
    public void updateEntity() {
        ...
        time--;
        if (time % 20 == 0) notifyBlockUpdate();
    }
