...
@Optional.InterfaceList({
    @Optional.Interface(
        modid = ModIDs.GALACTICRAFT_CORE,
        iface = "micdoodle8.mods.galacticraft.api.power.IEnergyHandlerGC"),
    @Optional.Interface(
        modid = ModIDs.GALACTICRAFT_CORE,
        iface = "micdoodle8.mods.galacticraft.api.transmission.tile.IElectrical") })
public class TileEntityAFSU extends TileEntityEnergyStorage implements IEnergyStorage, IEnergyHandlerGC, IElectrical {
    ...
    public void setRedstoneMode(byte value) {
        ...
        redstoneMode = value;
        if (worldObj != null && !worldObj.isRemote && redstoneMode != old) notifyBlockUpdate();
    }
    ...
    protected boolean shouldEmitEnergy() {
        ...
        if (redstoneMode == 5) return !redstone;
        if (redstoneMode == 6) return (!redstone || energy > capacity - output * 20.0D);
        return true;
    }
    ...
    public void onServerMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                if (tag.hasKey("value")) setRedstoneMode((byte) tag.getDouble("value"));
                break;
        }
    }
    ...
    public void onClientMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                if (tag.hasKey("value")) energy = tag.getDouble("value");
                break;
        }
    }
    ...
    public void updateState() {
        ...
        if (!worldObj.isRemote && (poweredBlock != old || allowEmit != shouldEmitEnergy())) {
            ...
            if (poweredBlock != old) worldObj.notifyBlocksOfNeighborChange(xCoord, yCoord, zCoord, block);
