...
public class GuiHowlerAlarmListBox extends GuiButton {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
        EnergyControl.MODID,
        "textures/gui/gui_howler_alarm.png");
    ...
    public GuiHowlerAlarmListBox(int id, int left, int top, int width, int height, List<String> items,
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        ...
        GL11.glScissor(
            xPosition * scaler.getScaleFactor(),
        ...
        for (String row : items) {
            if (row.equals(currentItem)) {
                drawRect(
                    xPosition,
                    yPosition + rowTop - scrollTop - 1,
                    xPosition + width - SCROLL_WIDTH,
                    yPosition + rowTop - scrollTop + lineHeight - 1,
                    selectedColor);
                fontRenderer
                    .drawString(row, xPosition + BASIC_X_OFFSET, yPosition + rowTop - scrollTop, selectedFontColor);
            } else fontRenderer.drawString(row, xPosition + BASIC_X_OFFSET, yPosition + rowTop - scrollTop, fontColor);
        ...
        sliderY = yPosition + SCROLL_BUTTON_HEIGHT
    ...
    private void setCurrent(int targetY) {
        if (lineHeight == 0 || items.size() == 0) return;
    ...
    public boolean mousePressed(Minecraft mc, int mouseX, int mouseY) {
        if (super.mousePressed(mc, mouseX, mouseY)) {
            if (mouseX > xPosition + width - SCROLL_WIDTH) {// scroll click
                if (mouseY - yPosition < SCROLL_BUTTON_HEIGHT) scrollUp();
                else if (height + yPosition - mouseY < SCROLL_BUTTON_HEIGHT) scrollDown();
