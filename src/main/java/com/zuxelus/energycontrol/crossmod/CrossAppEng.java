...
public class CrossAppEng extends CrossModBase {
    ...
    public NBTTagCompound getCardData(TileEntity te) {
        ...
        if (te instanceof TileCraftingMonitorTile) {
            ...
            if (tile.getJobProgress() != null) {
                ...
                tag.setString(
                    "name",
                    EnergyControl.proxy.getItemName(
                        tile.getJobProgress()
                            .getItemStack()));
                tag.setInteger(
                    "size",
                    (int) tile.getJobProgress()
                        .getStackSize());
            }
            return tag;
        }

        if (te instanceof IGridProxyable) list = ((IGridProxyable) te).getProxy()
            .getNode()
            .getGrid()
            .getNodes();
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_APPENG,
            new Object[] { "IT", "PD", 'P', Items.paper, 'D', "dyePurple", 'T',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'I', AEApi.instance()
                    .definitions()
                    .materials()
                    .fluixCrystal()
                    .maybeStack(1)
