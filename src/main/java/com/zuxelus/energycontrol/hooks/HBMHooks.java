...
public class HBMHooks {
    ...
    public static void updateEntity(TileEntityMachineBattery te) {
        ...
        if (values != null && values.size() > 0) {
            for (int i = 20; i > 0; i--) values.set(i, values.get(i - 1));
            values.set(0, te.power);
        } else {
            values = new ArrayList<>();
            for (int i = 0; i < 21; i++) values.add(te.power);
            map.put(te, values);
        }
    }
    ...
    public static void burn(TileEntityCore te, long joules) {
        ...
        if (te.tanks[0].getFill() < demand || te.tanks[1].getFill() < demand) return;
    ...
    public static void makePower(TileEntityMachineTurbineGas te, double consMax, int throttle) {
        ...
        long energy = 0L;
        if (te.tanks[0].getTankType()
            .hasTrait(FT_Combustible.class))
            energy = ((FT_Combustible) te.tanks[0].getTankType()
                .getTrait(FT_Combustible.class)).getCombustionEnergy() / 1000L;
