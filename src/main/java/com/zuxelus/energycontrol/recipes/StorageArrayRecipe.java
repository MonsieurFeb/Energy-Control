...
public class StorageArrayRecipe implements IRecipe {
    ...
    static {
        RecipeSorter.register(
            "EnergyControl:storagearrayRecipe",
            StorageArrayRecipe.class,
    ...
    public ItemStack getCraftingResult(InventoryCrafting inventory) {
        ...
        for (int i = 0; i < inventoryLength; i++) {
            ...
            if (!ItemCardMain.isCard(stack)) return null;
        ...
        if ((cardCount + arrayCount) != 0 && (cardCountLiquid + arrayCountLiquid) != 0) return null;
    ...
    private ItemStack getCraftingResult(int cardCount, int arrayCount, int type, Vector<ItemStack> cards,
        ItemStack array) {
        ...
        if (cardCount == 0 && arrayCount == 1) {
            ...
            if (cnt > 0) return new ItemStack(ModItems.itemComponent, cnt, ItemComponent.BASIC_CIRCUIT);
        } else if (arrayCount == 1 && cardCount > 0) {
            int cnt = new ItemCardReader(array).getInt("cardCount");
            if (cnt + cardCount <= 16) {
                ItemStack itemStack = new ItemStack(ModItems.itemCard, 1, type);
                itemStack.setTagCompound(
                    (NBTTagCompound) array.getTagCompound()
                        .copy());
                initArray(itemStack, cards);
                return itemStack;
            }
    ...
    private static void initArray(ItemStack stack, Vector<ItemStack> cards) {
        if (stack.getItemDamage() != ItemCardType.CARD_ENERGY_ARRAY
            ...
            && stack.getItemDamage() != ItemCardType.CARD_GENERATOR_ARRAY) return;
