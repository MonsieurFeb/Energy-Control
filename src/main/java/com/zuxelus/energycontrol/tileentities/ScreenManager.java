...
public class ScreenManager {
    ...
    private int getWorldKey(World world) {
        ...
        if (world.provider == null) return -10;
    ...
    public void clearWorld(World world) {
        ...
        if (screens.containsKey(key)) screens.get(key)
            .clear();
        if (unusedPanels.containsKey(key)) unusedPanels.get(key)
            .clear();
    }
    ...
    public void registerInfoPanel(TileEntityInfoPanel panel) {
        ...
        for (Screen screen : screens.get(getWorldKey(world))) if (screen.isBlockPartOf(panel)) {
            destroyScreen(screen, world); // occurs on chunk unloading/loading
            break;
        }

        Screen screen = buildFromPanel(panel);
        screens.get(getWorldKey(world))
            .add(screen);
    }
            ...
            for (int interX = 0; interX <= rx && allOk; interX++) {
                for (int interY = 0; interY <= ry && allOk; interY++) {
                    for (int interZ = 0; interZ <= rz && allOk; interZ++) {
                        ...
                        allOk = core != null && isValidExtender(
                            world,
                            x + dir * interX,
                            y + dir * interY,
                            z + dir * interZ,
    ...
    private boolean isValidExtender(World world, int x, int y, int z, ForgeDirection facing, boolean advanced,
        ...
        if (!(block instanceof InfoPanelExtender || block instanceof HoloPanelExtender)) return false;
        TileEntity te = world.getTileEntity(x, y, z);
        if (!(te instanceof TileEntityInfoPanelExtender)) return false;
        if (advanced ^ (te instanceof TileEntityAdvancedInfoPanelExtender)) return false;
        if (holo ^ (te instanceof TileEntityHoloPanelExtender)) return false;
        if (((TileEntityInfoPanelExtender) te).getFacingForge() != facing) return false;
        return ((IScreenPart) te).getScreen() == null;
    }

    public Screen loadScreen(TileEntityInfoPanel panel) {
        if (panel.screenData == null) return null;
    ...
    public void registerInfoPanelExtender(TileEntityInfoPanelExtender extender) {
        if (extender.getWorldObj().isRemote) return;
        ...
        for (Screen screen : screensToDestroy) destroyScreen(screen, extender.getWorldObj());
        ...
        for (TileEntityInfoPanel panel : rebuildPanels) {
            Screen screen = buildFromPanel(panel);
            screens.get(getWorldKey(extender.getWorldObj()))
                .add(screen);
    ...
    public void unregisterScreenPart(TileEntity part) {
        if (part.getWorldObj().isRemote) return;
        ...
        if (screen == null) {
            if (part instanceof TileEntityInfoPanel) unusedPanels.get(getWorldKey(part.getWorldObj()))
                .remove(part);
            return;
        }
        ...
        if (!isCoreDestroyed && core != null) {
            ...
            screens.get(getWorldKey(core.getWorldObj()))
                .add(newScreen);
        }
    }
}
