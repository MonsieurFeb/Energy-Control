...
public class ItemCardToggle extends ItemCardBase implements ITouchAction {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        if (block == null) return CardState.NO_TARGET;
    ...
    public boolean runTouchAction(World world, ICardReader reader, ItemStack stack) {
        ...
        if (block == Blocks.lever) {
            ...
            if (meta == 1) world.notifyBlocksOfNeighborChange(pos.posX - 1, pos.posY, pos.posZ, block);
            else if (meta == 2) world.notifyBlocksOfNeighborChange(pos.posX + 1, pos.posY, pos.posZ, block);
            else if (meta == 3) world.notifyBlocksOfNeighborChange(pos.posX, pos.posY, pos.posZ - 1, block);
            else if (meta == 4) world.notifyBlocksOfNeighborChange(pos.posX, pos.posY, pos.posZ + 1, block);
            else if (meta != 5 && meta != 6) {
                if (meta == 0 || meta == 7) world.notifyBlocksOfNeighborChange(pos.posX, pos.posY + 1, pos.posZ, block);
            } else world.notifyBlocksOfNeighborChange(pos.posX, pos.posY - 1, pos.posZ, block);
        }
    ...
    public void renderImage(TextureManager manager, ICardReader reader) {
        ...
        else manager.bindTexture(new ResourceLocation(EnergyControl.MODID + ":textures/gui/grey.png"));
