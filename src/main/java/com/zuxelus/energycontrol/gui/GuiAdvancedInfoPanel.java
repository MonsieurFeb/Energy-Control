...
public class GuiAdvancedInfoPanel extends GuiPanelBase {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
        EnergyControl.MODID,
        "textures/gui/gui_advanced_info_panel.png");
    ...
    protected void initButtons() {
        buttonList.add(
            new GuiButtonGeneral(
                ...
                TEXTURE,
                176,
                panel.getShowLabels() ? 15 : 31).setGradient());
        buttonList.add(
            new GuiButtonGeneral(ID_SLOPE, guiLeft + 83 + 17, guiTop + 42, 16, 16, TEXTURE, 192, 15).setGradient());
        buttonList.add(
            new GuiButtonGeneral(ID_COLORS, guiLeft + 83 + 17 * 2, guiTop + 42, 16, 16, TEXTURE, 192, 28).setGradient()
                .setScale(2));
        buttonList.add(
            new GuiButtonGeneral(
                ID_POWER,
                ...
                TEXTURE,
                192 - 16,
                getIconPowerTopOffset(((TileEntityAdvancedInfoPanel) panel).getPowerMode())).setGradient());
        buttonList.add(
            new GuiButtonGeneral(
                ID_TICKRATE,
                guiLeft + 83 + 17 * 4,
                ...
                16,
                16,
                Integer.toString(panel.getTickRate())).setGradient());
    }

    @SuppressWarnings("unchecked")
    @Override
    protected void initControls() {
        ItemStack stack = panel.getCards()
            .get(activeTab);
        if (ItemStack.areItemStacksEqual(stack, oldStack)) return;
        if (oldStack != null && stack == null) updateTitle();
        if (stack == null) // for 1.10 and less
            oldStack = null;
        else oldStack = stack.copy();
        buttonList.clear();
        initButtons();
        if (ItemCardMain.isCard(stack)) {
            int slot = panel.getCardSlot(stack);
            if (stack.getItemDamage() == ItemCardType.CARD_TEXT) buttonList
                .add(new GuiButtonGeneral(ID_TEXT, guiLeft + 83 + 17 * 4, guiTop + 42, 16, 16, "txt").setGradient());
