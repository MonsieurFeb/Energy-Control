...
public class InfoPanel extends FacingBlock {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int facing, float hitX,
        ...
        if (!(te instanceof TileEntityInfoPanel)) return true;
        if (!world.isRemote && EnergyControl.altPressed.get(player) && ((TileEntityInfoPanel) te).getFacing() == facing)
            if (((TileEntityInfoPanel) te).runTouchAction(player.getHeldItem(), x, y, z, hitX, hitY, hitZ)) return true;
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        ...
        TileEntity te = world.getTileEntity(x, y, z);
        if (!(te instanceof TileEntityInfoPanel)) return;
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (be instanceof TileEntityInfoPanel) ((TileEntityInfoPanel) be).updateExtenders(meta > 5);
    }

    @Override
    public boolean canConnectRedstone(IBlockAccess world, int x, int y, int z, int side) {
        TileEntity te = world.getTileEntity(x, y, z);
        if (te instanceof TileEntityFacing) return side == ((TileEntityFacing) te).getFacingForge()
            .ordinal();
        return false;
    }
