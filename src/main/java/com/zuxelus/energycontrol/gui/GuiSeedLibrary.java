...
public class GuiSeedLibrary extends GuiContainer {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
        EnergyControl.MODID,
        "textures/gui/gui_seed_library.png");
    ...
    public void initGui() {
        ...
        unk_type_button = new GuiButtonImage(
            2,
            guiLeft + left + main_width / 8 - 9,
            guiTop + middle + 20,
            18,
            20,
            176,
        ...
        unk_ggr_button = new GuiButtonImage(
            3,
            guiLeft + left + (main_width * 3) / 8 - 9,
            guiTop + middle + 20,
            18,
            20,
            176,
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        drawCenteredString(
            I18n.format(filter.getCropName()),
            left + main_width / 4,
            top + 2 + 8 + 1 + 18 + 2,
            0x404040);

        String count;
        if (container.te.seeds_available >= 65535) count = "MANY";
        else count = container.te.seeds_available + "";
        ...
        if (lastMouseX != mouseX || lastMouseY != mouseY) onMouseMoved(mouseX, mouseY);
    ...
    protected void mouseMovedOrUp(int mouseX, int mouseY, int state) {
        ...
        if (state == 0)
            ...
            if (current_slider != -1) current_slider = -1;
    }
    ...
    private void onMouseMoved(int newX, int newY) {
        ...
        if (is_max && min > value) value = min;
        else if (!is_max && max < value) value = max;

        if (container.te.getSliderValue(current_slider) != value) setSliderValue(current_slider, value);
    }

    private String getTooltip(int x, int y) {
        int slider = current_slider;
        if (slider == -1) slider = getSliderAt(x, y);
