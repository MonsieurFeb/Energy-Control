...
public class CrossGregTech6 extends CrossModBase {
    ...
    public NBTTagCompound getCardData(TileEntity te) {
        ...
        if (te instanceof MultiTileEntityBatteryBox) {
            ...
            for (ItemStack tStack : box.getInventory())
                if (ST.valid(tStack)) if (tStack.getItem() instanceof IItemEnergy) {
                    energy = energy + ((IItemEnergy) tStack.getItem()).getEnergyStored(box.mEnergyType, tStack);
                }
            tag.setDouble("battery", energy);
            tag.setDouble(
                "limit0",
                ((MultiTileEntityBatteryBox) te).mInput * 2 * 40 * ((MultiTileEntityBatteryBox) te).invsize());
            tag.setDouble(
                "limit1",
                ((MultiTileEntityBatteryBox) te).mInput * 6 * 40 * ((MultiTileEntityBatteryBox) te).invsize());
            tag.setDouble(
                DataHelper.CAPACITY,
                ((MultiTileEntityBatteryBox) te).mInput * 8 * 40 * ((MultiTileEntityBatteryBox) te).invsize());
        ...
        if (te instanceof MultiTileEntityGeneratorMetal) {
            ...
            tag.setDouble(
                DataHelper.OUTPUTHU,
                active ? DataHelper.getLong(MultiTileEntityGeneratorSolid.class, "mRate", te) : 0);
            ItemStack[] list = ((MultiTileEntityGeneratorMetal) te).getInventory();
            if (list[0] != null) tag.setTag("slot0", list[0].writeToNBT(new NBTTagCompound()));
            if (list[1] != null) tag.setTag("slot1", list[1].writeToNBT(new NBTTagCompound()));
            return tag;
        }
            ...
            try {
                ...
                for (OreDictMaterialStack item : list) {
                    ...
                    builder.append(
                        String.format(
                            "%s: %s",
                            item.mMaterial.mNameLocal,
                            PanelString.getFormatter()
                                .format(item.mAmount / 648648000d)));
                }
        ...
        if (te instanceof MultiTileEntityAxle) {
            ...
            if (values != null) {
                long energy = 0;
                for (int i = 0; i < 20; i++) energy = energy + values.get(i);
        ...
        if (te instanceof MultiTileEntityBoilerTank) {
            ...
            if (values != null) {
                ...
                long energy = 0;
                for (int i = 0; i < 60; i++) {
                    if (i < 20) water = water + values.get(i);
        ...
        if (te instanceof MultiTileEntityReactorCore2x2) {
            ...
            for (ItemStack stack : DataHelper.getItemStackList(TileEntityBase05Inventories.class, "mInventory", te)) {
                if (stack != null && stack.hasTagCompound()) {
                    ...
                    if (value > 0 && value < dur) dur = value;
                }
            }
            if (dur == Long.MAX_VALUE) dur = 0;
    ...
    public ArrayList getHookValues(TileEntity te) {
        ...
        if (values == null) GregTechHooks.map.put(te, null);
        return values;
    }
    ...
    public static double getAverage(ArrayList<Long> values, int start, int size) {
        ...
        double sum = 0;
        for (int i = start; i < start + size; i++) sum = sum + values.get(i);
    ...
    public void addHookData(TileEntity te, NBTTagCompound tag, String consumption, String output, boolean showPackage) {
        ...
        if (output != null) tag.setDouble(output, getAverage(values, GregTechHooks.len, GregTechHooks.len));
    }
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_GREGTECH,
            new Object[] { "RF", "PB", 'P', Items.paper, 'R', "dyeGray", 'F',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'B', "ingotBronze" });
