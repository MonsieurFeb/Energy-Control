...
public class ItemCardEnergyArray extends ItemCardBase {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        if (cardCount == 0) return CardState.INVALID_CARD;
            ...
            if (Math.abs(dx) <= range && Math.abs(dy) <= range && Math.abs(dz) <= range) {
                ...
                if (te != null) {
                    ...
                    if (tag != null) {
                        ...
                        foundAny = true;
                    } else reader.setInt(String.format("_%denergy", i), STATUS_NOT_FOUND);
                } else reader.setInt(String.format("_%denergy", i), STATUS_NOT_FOUND);
            } else {
    ...
    @Override
    public List<PanelString> getStringData(int displaySettings, ICardReader reader, boolean isServer,
        boolean showLabels) {
        ...
        for (int i = 0; i < reader.getCardCount(); i++) {
            ...
            if (showEach) {
                if (isOutOfRange) {
                    ...
                    if (showEnergy) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelEnergyN",
                                    i + 1,
                                    StringUtils.getFormatted("", energy, false))));
                        else result.add(new PanelString(StringUtils.getFormatted("", energy, false)));
                    }
                    if (showFree) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelFreeN",
                                    i + 1,
                                    StringUtils.getFormatted("", storage - energy, false))));
                        else result.add(new PanelString(StringUtils.getFormatted("", storage - energy, false)));
                    }
                    if (showStorage) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelStorageN",
                                    i + 1,
                                    StringUtils.getFormatted("", storage, false))));
                        else result.add(new PanelString(StringUtils.getFormatted("", storage, false)));
                    }
                    if (showPercentage) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelPercentageN",
                                    i + 1,
        ...
        if (showSummary) {
            if (showEnergy) result.add(new PanelString("msg.ec.InfoPanelEnergy", totalEnergy, showLabels));
            if (showFree) result.add(new PanelString("msg.ec.InfoPanelFree", totalStorage - totalEnergy, showLabels));
            if (showStorage) result.add(new PanelString("msg.ec.InfoPanelCapacity", totalStorage, showLabels));
            if (showPercentage) result.add(
                new PanelString(
                    "msg.ec.InfoPanelPercentage",
