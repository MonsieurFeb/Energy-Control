...
public class ItemCardInventory extends ItemCardBase {
        ...
        if (te instanceof IInventory && !(te instanceof TileEntityInfoPanel)) {
            ...
            for (int i = 0; i < inv.getSizeInventory(); i++) {
                ...
                if (stack != null) {
                    ...
                    if (i < 4) reader.setTag("slot" + i, stack.writeToNBT(new NBTTagCompound()));
                }
            }
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        if (isServer) {
            ...
            result.add(
                new PanelString(
                    "msg.ec.InfoPanelSlotsUsed",
                    String.format("%s/%s", reader.getInt("used"), reader.getInt("size")),
                    showLabels));
        } else {
            ...
            result.add(
                new PanelString(
                    "msg.ec.InfoPanelSidedInventory",
                    reader.getBoolean("sided")
                        .toString(),
                    showLabels));
            if (reader.hasField("slot0")) result.add(
                new PanelString(
                    String.format("msg.ec.InfoPanelSlot%d", 1),
                    StringUtils.getItemName(ItemStack.loadItemStackFromNBT(reader.getTag("slot0"))),
                    showLabels));
            if (reader.hasField("slot1")) result.add(
                new PanelString(
                    String.format("msg.ec.InfoPanelSlot%d", 2),
                    StringUtils.getItemName(ItemStack.loadItemStackFromNBT(reader.getTag("slot1"))),
                    showLabels));
            if (reader.hasField("slot2")) result.add(
                new PanelString(
                    String.format("msg.ec.InfoPanelSlot%d", 3),
                    StringUtils.getItemName(ItemStack.loadItemStackFromNBT(reader.getTag("slot2"))),
                    showLabels));
            if (reader.hasField("slot3")) result.add(
                new PanelString(
                    String.format("msg.ec.InfoPanelSlot%d", 4),
                    StringUtils.getItemName(ItemStack.loadItemStackFromNBT(reader.getTag("slot3"))),
                    showLabels));
        }
        return result;
    }
