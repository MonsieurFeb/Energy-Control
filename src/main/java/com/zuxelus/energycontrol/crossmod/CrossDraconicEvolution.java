...
public class CrossDraconicEvolution extends CrossModBase {
    ...
    public NBTTagCompound getEnergyData(TileEntity te) {
        ...
        if (te instanceof TileRemoteEnergyBase) {
            tag.setDouble(
                DataHelper.ENERGY,
                ((TileRemoteEnergyBase) te).getStorage()
                    .getEnergyStored());
            tag.setDouble(
                DataHelper.CAPACITY,
                ((TileRemoteEnergyBase) te).getStorage()
    ...
    public NBTTagCompound getCardData(TileEntity te) {
        NBTTagCompound tag = new NBTTagCompound();
        if (te instanceof TileInvisibleMultiblock) return getStorageData(((TileInvisibleMultiblock) te).getMaster());
        if (te instanceof TileEnergyStorageCore) return getStorageData((TileEnergyStorageCore) te);
        if (te instanceof TileEnergyInfuser) {
            tag.setDouble(DataHelper.ENERGY, ((TileEnergyInfuser) te).energy.getEnergyStored());
            tag.setDouble(DataHelper.CAPACITY, ((TileEnergyInfuser) te).energy.getMaxEnergyStored());
            return tag;
        ...
        if (te instanceof TileReactorCore) return getReactorData((TileReactorCore) te);
        return null;
    }
        ...
        tag.setString(
            ...
            (reactor.reactorState == 0 ? EnumChatFormatting.DARK_GRAY.toString()
                : reactor.reactorState == 1 ? EnumChatFormatting.RED.toString()
                    : reactor.reactorState == 2 ? EnumChatFormatting.DARK_GREEN.toString()
    ...
    public ArrayList getHookValues(TileEntity te) {
        ...
        if (values == null) DraconicEvolutionHooks.map.put(te, null);
        return values;
    }
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_DRACONIC_EVOLUTION,
            new Object[] { "RF", "PB", 'P', Items.paper, 'R', "dustRedstone", 'F',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'B', "ingotDraconium" });
