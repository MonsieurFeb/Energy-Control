...
public class ContainerTimer extends Container {
    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < crafters.size(); i++) {
            if (lastTime != time) NetworkHelper
                .updateClientTileEntity((ICrafting) crafters.get(i), te.xCoord, te.yCoord, te.zCoord, 1, time);
            if (lastIsWorking != isWorking) NetworkHelper.updateClientTileEntity(
