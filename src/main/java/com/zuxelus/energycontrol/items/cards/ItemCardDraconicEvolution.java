...
public class ItemCardDraconicEvolution extends ItemCardBase {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        NBTTagCompound tag = CrossModLoader.getCrossMod(ModIDs.DRACONIC_EVOLUTION)
            .getCardData(te);
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        if (reader.hasField("fuelRate") && (settings & 16) > 0) result.add(
            new PanelString(
                "msg.ec.InfoPanelConversionRate",
                reader.getDouble("fuelRate") * 1000000,
                "nb/t",
                showLabels));
