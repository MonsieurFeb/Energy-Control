...
public class IC2ReactorHelper {
    ...
    public static IReactor getReactorAt(World world, int x, int y, int z) {
        ...
        if (entity instanceof IReactorChamber) return ((IReactorChamber) entity).getReactor();
        return null;
    }
    ...
    public static IReactor getReactor3x3(World world, int x, int y, int z) {
        ...
        for (int xoffset = -1; xoffset < 2; xoffset++)
            for (int yoffset = -1; yoffset < 2; yoffset++) for (int zoffset = -1; zoffset < 2; zoffset++) {
                TileEntity te = world.getTileEntity(x + xoffset, y + yoffset, z + zoffset);
                if (te instanceof IReactor) return (IReactor) te;
                if (te instanceof IReactorChamber) return ((IReactorChamber) te).getReactor();
            }
        return null;
    }
    ...
    public static ChunkCoordinates getTargetCoordinates(World world, int x, int y, int z) {
        IReactor reactor = getReactorAt(world, x, y, z);
        if (reactor != null) return reactor.getPosition();
        return null;
    }
    ...
    public static void energyLoadEvent(TileEntity te, boolean isLoad) {
        if (isLoad) MinecraftForge.EVENT_BUS.post(new EnergyTileLoadEvent((IEnergyTile) te));
        else MinecraftForge.EVENT_BUS.post(new EnergyTileUnloadEvent((IEnergyTile) te));
    }
}
