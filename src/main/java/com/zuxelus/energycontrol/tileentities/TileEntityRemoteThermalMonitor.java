...
public class TileEntityRemoteThermalMonitor extends TileEntityThermalMonitor implements ISlotItemFilter {
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("heat")) heat = tag.getInteger("heat");
    }
        ...
        if (getStackInSlot(SLOT_CARD) != null) {
            ...
            if (target != null) {
                ...
                if (stack != null && stack.getItem() instanceof ItemUpgrade
                    && stack.getItemDamage() == ItemUpgrade.DAMAGE_RANGE) upgradeCountRange = stack.stackSize;
                int range = LOCATION_RANGE * (int) Math.pow(2, upgradeCountRange);
                if (Math.abs(target.posX - xCoord) <= range && Math.abs(target.posY - yCoord) <= range
                    && Math.abs(target.posZ - zCoord) <= range) {
