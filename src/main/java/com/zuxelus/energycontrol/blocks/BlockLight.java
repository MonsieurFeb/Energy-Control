...
public class BlockLight extends Block {
    ...
    private void updateBlockState(World world, int x, int y, int z) {
        ...
        int meta = world.getBlockMetadata(x, y, z);
        if (meta % 2 == 1) {
            if (!world.isBlockIndirectlyGettingPowered(x, y, z)) world.setBlockMetadataWithNotify(x, y, z, meta - 1, 2);
        } else
            if (world.isBlockIndirectlyGettingPowered(x, y, z)) world.setBlockMetadataWithNotify(x, y, z, meta + 1, 2);
    }
    ...
    public void getSubBlocks(Item id, CreativeTabs tab, List items) {
        for (int i = 0; i <= DAMAGE_MAX; i++) if (i % 2 == 0) items.add(new ItemStack(this, 1, i));
    }
}
