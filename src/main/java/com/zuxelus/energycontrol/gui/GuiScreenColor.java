...
public class GuiScreenColor extends GuiBase {
    ...
    private final static ResourceLocation PICKER = new ResourceLocation(
        EnergyControl.MODID + ":textures/gui/gui_color_picker.png");
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        for (GuiTextNumeric text : fieldList) text.drawTextBox();
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseButton == 0) {
            for (GuiTextNumeric text : fieldList) text.mouseClicked(mouseX - guiLeft, mouseY - guiTop, mouseButton);
    ...
    private void checkColorPicker(int mouseX, int mouseY) {
        ...
        if (isInside(mouseX, mouseY, 20, 34, 80, 80)) {
            ...
            if (x <= 0 && y >= 0) hue = 180 - hue;
            else if (x <= 0 && y <= 0) hue = 180 + hue;
            else if (x >= 0 && y <= 0) hue = 360 - hue;
            if (hue > 360) hue = 359;
            Color c = isDarkPicker ? getColorFromHSV(hue, 1.0F, saturation) : getColorFromHSV(hue, saturation, 1.0F);
            setColorText(c);
            fieldList.get(0)
                .setText(Integer.toString(c.getRed()));
            fieldList.get(1)
                .setText(Integer.toString(c.getGreen()));
            fieldList.get(2)
                .setText(Integer.toString(c.getBlue()));
            /*
    ...
    private Color getColorFromHSV(int hue, float saturation, float value) {
        ...
        float m = value - c;
        if (hue < 60) return new Color(c + m, x + m, m);
        if (hue < 120) return new Color(x + m, c + m, m);
        if (hue < 180) return new Color(m, c + m, x + m);
        if (hue < 240) return new Color(m, x + m, c + m);
        if (hue < 300) return new Color(x + m, m, c + m);
    ...
    private int getScreenColor(int mouseX, int mouseY) {
        if (!isInside(mouseX, mouseY, 160, 23, 52, 46)) return -1;
        for (int i = 0; i < 5; i++)
            for (int j = 0; j < 5; j++) if (isInside(mouseX, mouseY, 160 + j * 14, 23 + i * 14, 8, 8)) return i * 4 + j;
        return -1;
    }
    ...
    public void updateScreen() {
        super.updateScreen();
        for (GuiTextNumeric text : fieldList) text.updateCursorCounter();
    }

    @Override
    protected void keyTyped(char typedChar, int keyCode) {
        if (keyCode == 1) FMLClientHandler.instance()
            .getClient()
            .displayGuiScreen(parentGui);
        else if (keyCode == 15) {
            if (fieldList.get(0)
                .isFocused()) {
                fieldList.get(0)
                ...
                fieldList.get(1)
                    .setFocused(true);
            } else if (fieldList.get(1)
                .isFocused()) {
                    fieldList.get(1)
                        .setFocused(false);
                    fieldList.get(2)
                        .setFocused(true);
                } else if (fieldList.get(2)
                    .isFocused()) {
                        fieldList.get(2)
                            .setFocused(false);
                        fieldList.get(0)
        ...
        } else for (GuiTextNumeric text : fieldList) {
