...
public class TEAdvancedInfoPanelRenderer extends TileEntitySpecialRenderer {
    ...
    static {
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++) model[i * 4 + j] = new CubeRenderer(i * 32 + 64, j * 32 + 64);
    }
    ...
    public void renderTileEntityAt(TileEntityAdvancedInfoPanel te, double x, double y, double z) {
        ...
        if (te.getColored()) {
            ...
            if (color > 15 || color < 0) color = TileEntityAdvancedInfoPanel.DEFAULT_BACKGROUND;
        }
        if (te.getPowered()) bindTexture(TEXTUREON[color]);
        else bindTexture(TEXTUREOFF[color]);

        int textureId = te.findTexture();
        byte thickness = te.thickness;
        if (thickness < 1 || thickness > 16) thickness = 16;
        ...
        if (screen != null) {
            if (thickness == 16 && rotateHor == 0 && rotateVert == 0) model[textureId].render(0.03125F);
            else new CubeRenderer(
                textureId / 4 * 32 + 64,
                textureId % 4 * 32 + 64,
                offset.addOffset(screen, te.xCoord, te.yCoord, te.zCoord, te.getFacingForge(), te.getRotation()))
                    .render(0.03125F);
            if (te.powered) {
                List<PanelString> joinedData = te.getPanelStringList(false, te.getShowLabels());
                if (joinedData != null) drawText(te, joinedData, thickness, offset);
            }
        }
        GL11.glPopMatrix();
    }
    ...
    private void drawText(TileEntityAdvancedInfoPanel panel, List<PanelString> joinedData, byte thickness,
        ...
        GL11.glRotated(
            90.0D - Math.toDegrees( // Law of cosines
                Math.acos(
                    (h * h + v * v) / 2
                        / Math.sqrt(displayWidth * displayWidth + h * h)
                        / Math.sqrt(displayHeight * displayHeight + v * v))),
            0.0D,
            0.0D,
            i * j);
        ...
        if (panel.getColored()) colorHex = panel.getColorText();
        for (PanelString panelString : joinedData) {
