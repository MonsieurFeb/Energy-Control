...
public class TileEntityAverageCounter extends TileEntityEnergyStorage {
    ...
    public double getClientAverage() {
        if (clientAverage == -1) return getAverage();
    ...
    private double getAverage() {
        ...
        double sum = 0;
        for (int i = 0; i < period * 20; i++) sum += data[(start + i) % DATA_POINTS];
    ...
    private void setPeriod(short p) {
        period = p;
        if (!worldObj.isRemote && prevPeriod != period) notifyBlockUpdate();
        prevPeriod = period;
    }
    ...
    public void onClientMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                if (tag.hasKey("value")) clientAverage = tag.getDouble("value");
                break;
        }
    }
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        for (int i = 0; i < DATA_POINTS; i++) data[i] = tag.getDouble("point-" + i);
    }
    ...
    protected NBTTagCompound writeProperties(NBTTagCompound tag) {
        ...
        for (int i = 0; i < DATA_POINTS; i++) tag.setDouble("point-" + i, data[i]);
        return tag;
    }
    ...
    private void refreshData() {
        ...
        if (stack != null && stack.isItemEqual(
            CrossModLoader.getCrossMod(ModIDs.IC2)
                .getItemStack("transformer")))
