...
public class SeedLibraryFilter {
    ...
    public ItemStack getSeed(Collection<ItemStack> seeds) {
        if (cached_nothing) return null;

        if (cache.size() == 0) fillCache(seeds);
    ...
    public int getCount(Collection<ItemStack> seeds) {
        int count = 0;
        for (ItemStack seed : seeds) if (isMatch(seed)) count += seed.stackSize;
    ...
    public void settingsChanged() {
        ...
        if (te != null && !FMLCommonHandler.instance()
            .getEffectiveSide()
    ...
    public boolean isMatch(ItemStack seed) {
        int id = Crops.instance.getCropCard(seed)
            .getId();
        byte scan = ItemCropSeed.getScannedFromStack(seed);

        if (scan == 0) return (unknown_type > 0) && (unknown_ggr > 0);
        if (unknown_type == 2) return false;
        if (seed_type != -1 && seed_type != id) return false;
        if (scan < 4) return (unknown_ggr > 0);
        if (unknown_ggr == 2) return false;
        ...
        if (growth < min_growth || growth > max_growth
            ...
            || resistance > max_resistance) return false;
        int total = growth + gain + resistance;
        if (total < min_total || total > max_total) return false;
    ...
    public String getCropName() {
        ...
        if (crop_id == -1) return "Any";
        if (Crops.instance.getCropList()[crop_id] == null) return "(Invalid)";
    ...
    public void setCropFromSeed(ItemStack seed) {
        if (seed == null) {
            seed_type = -1;
        } else if (seed.getItem() != IC2Items.getItem("cropSeed")
            .getItem()) {
                seed_type = -1;
    ...
    protected void fillCache(Collection<ItemStack> seeds) {
        ...
        bulk_mode = true;
        for (ItemStack seed : seeds) newSeed(seed);
        bulk_mode = false;
    ...
    protected void updateSeedCount() {
        if (!bulk_mode && te != null) te.updateSeedCount();
    }
    ...
    protected void addToCache(ItemStack seed) {
        ...
        if (pos >= CACHE_SIZE) return;

        cache.add(pos, seed);

        while (cache.size() > CACHE_SIZE) cache.remove(cache.size() - 1);
