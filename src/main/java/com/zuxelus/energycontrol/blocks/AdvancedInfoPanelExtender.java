...
public class AdvancedInfoPanelExtender extends InfoPanelExtender {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int facing, float hitX,
        ...
        if (CrossModLoader.getCrossMod(ModIDs.IC2)
            .isWrench(player.getHeldItem())) return true;
        if (world.isRemote) return true;
        TileEntity te = world.getTileEntity(x, y, z);
        if (!(te instanceof TileEntityInfoPanelExtender)) return true;
        TileEntityInfoPanel panel = ((TileEntityInfoPanelExtender) te).getCore();
        if (panel instanceof TileEntityAdvancedInfoPanel) player.openGui(
    ...
    public float[] getBlockBounds(TileEntityFacing tile) {
        if (!(tile instanceof TileEntityAdvancedInfoPanelExtender)) return super.getBlockBounds(tile);

        TileEntityAdvancedInfoPanelExtender te = (TileEntityAdvancedInfoPanelExtender) tile;
        Screen screen = te.getScreen();
        if (screen == null) return super.getBlockBounds(tile);

        switch (te.getFacingForge()) {
            case EAST:
                return new float[] { 0.0F, 0.0F, 0.0F,
                    0.0625F * ((TileEntityAdvancedInfoPanelExtender) te).getThickness(), 1.0F, 1.0F };
            case WEST:
                return new float[] { 1.0F - 0.0625F * ((TileEntityAdvancedInfoPanelExtender) te).getThickness(), 0.0F,
                    0.0F, 1.0F, 1.0F, 1.0F };
            case SOUTH:
                return new float[] { 0.0F, 0.0F, 0.0F, 1.0F, 1.0F,
                    0.0625F * ((TileEntityAdvancedInfoPanelExtender) te).getThickness() };
            case NORTH:
                return new float[] { 0.0F, 0.0F,
                    1.0F - 0.0625F * ((TileEntityAdvancedInfoPanelExtender) te).getThickness(), 1.0F, 1.0F, 1.0F };
            case UP:
                return new float[] { 0.0F, 0.0F, 0.0F, 1.0F,
                    0.0625F * ((TileEntityAdvancedInfoPanelExtender) te).getThickness(), 1.0F };
            case DOWN:
                return new float[] { 0.0F, 1.0F - 0.0625F * ((TileEntityAdvancedInfoPanelExtender) te).getThickness(),
                    0.0F, 1.0F, 1.0F, 1.0F };
            default:
