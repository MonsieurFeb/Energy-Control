...
public class TileEntityKitAssembler extends TileEntityInventory
    implements ITilePacketHandler, ISlotItemFilter, IEnergySink, IEnergyReceiverMK2, IEnergyReceiver {
        ...
        switch (tag.getInteger("type")) {
            case 4:
                if (tag.hasKey("slot") && tag.hasKey("title")) {
                    ...
                    if (ItemCardMain.isCard(stack)) new ItemCardReader(stack).setTitle(tag.getString("title"));
                }
                break;
        }
    }
    ...
    public void onClientMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                ...
                if (tag.hasKey("time")) recipeTime = tag.getInteger("time");
                else recipeTime = 0;
                break;
        }
    }
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("energy")) storage.setEnergy(tag.getLong("energy"));
        if (tag.hasKey("buffer")) buffer = tag.getInteger("buffer");
        if (tag.hasKey("production")) production = tag.getDouble("production");
    ...
    public void updateEntity() {
        if (!addedToEnet && Loader.isModLoaded(ModIDs.IC2)) onLoad();
        if (worldObj.isRemote) return;
        handleDischarger(SLOT_DISCHARGER);
        if (Loader.isModLoaded(ModIDs.HBM)) for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS)
        ...
        if (storage.getEnergyStored() >= CONSUMPTION) {
            ...
            if (recipe != null && production >= recipe.time) {
                ...
                if (result == null) setInventorySlotContents(SLOT_RESULT, recipe.output.copy());
                else result.stackSize += recipe.output.stackSize;
                production = 0;
                updateState();
            }
        } else {
    ...
    private void handleDischarger(int slot) {
        ...
        if (stack != null && needed > 0) {
            ...
            if (CrossModLoader.isElectricItem(stack)) {
                ...
                if (!active && storage.getEnergyStored() > old) markDirty();
            }
        }
    }
    ...
    private void updateActive() {
        active = false;
        if (storage.getEnergyStored() < CONSUMPTION) return;
    ...
    private void updateState() {
        ...
        if (!(block instanceof KitAssembler) || meta > 5 == active) return;
    ...
    public boolean isItemValid(int slot, ItemStack stack) { // ISlotItemFilter
        switch (slot) {
            ...
            case SLOT_DISCHARGER:
                return CrossModLoader.isElectricItem(stack) || stack.getItem()
                    .equals(Items.lava_bucket);
            case SLOT_RESULT:
            default:
                return false;
    ...
    public int[] getAccessibleSlotsFromSide(int side) {
        if (side == ForgeDirection.UP.ordinal()) return new int[] { SLOT_CARD1, SLOT_ITEM, SLOT_CARD2 };
        if (side == ForgeDirection.DOWN.ordinal()) return new int[] { SLOT_RESULT };
