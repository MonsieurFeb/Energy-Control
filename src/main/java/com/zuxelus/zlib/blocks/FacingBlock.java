...
public abstract class FacingBlock extends BlockContainer {
    ...
    public int getMetaForPlacement(World world, int x, int y, int z, int facing, float hitX, float hitY, float hitZ,
        int meta, EntityPlayer placer) {
        if (placer.rotationPitch >= 65) return ForgeDirection.UP.ordinal();
        if (placer.rotationPitch <= -65) return ForgeDirection.DOWN.ordinal();
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase placer, ItemStack stack) {
        ...
        if (te instanceof TileEntityFacing) switch (((TileEntityFacing) te).getFacingForge()) {
            case UP:
            case DOWN:
    ...
    protected static ForgeDirection getHorizontalFacing(EntityLivingBase placer) { // 1.7.10, no HORIZONTALS in
                                                                                   // ForgeDirection
        switch (MathHelper.floor_double(placer.rotationYaw * 4.0F / 360.0F + 0.5D) & 3) {
            case 0:
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        TileEntity te = world.getTileEntity(x, y, z);
        if (te instanceof TileEntityInventory) ((TileEntityInventory) te).dropItems(world, x, y, z); // 1.7.10, no
                                                                                                     // InventoryHelper
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int facing, float hitX,
        ...
        if (!world.isRemote) player.openGui(EnergyControl.instance, getBlockGuiId(), world, x, y, z);
        return true;
    }
