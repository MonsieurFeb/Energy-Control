...
public abstract class TileEntityFacing extends TileEntity implements IWrenchable {
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("facing")) facing = ForgeDirection.getOrientation(tag.getInteger("facing"));
        else facing = ForgeDirection.NORTH;
        if (hasRotation()) {
            if (tag.hasKey("rotation")) rotation = ForgeDirection.getOrientation(tag.getInteger("rotation"));
            else rotation = ForgeDirection.NORTH;
        }
    }

    protected NBTTagCompound writeProperties(NBTTagCompound tag) {
        tag.setInteger("facing", facing.ordinal());
        if (hasRotation() && rotation != null) tag.setInteger("rotation", rotation.ordinal());
        return tag;
    }
    ...
    public void setFacing(short facing) {
        ...
        if (block instanceof FacingHorizontal && (facing == 0 || facing == 1)) return;
