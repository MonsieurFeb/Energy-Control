...
public class ItemCardGalacticraft extends ItemCardBase {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        if (te instanceof TileEntityRefinery) {
            ...
            reader.setString(
                "oilTank",
                String.format("%s / %s mB", ref.oilTank.getFluidAmount(), ref.oilTank.getCapacity()));
            reader.setString(
                "fuelTank",
                String.format("%s / %s mB", ref.fuelTank.getFluidAmount(), ref.fuelTank.getCapacity()));
            return CardState.OK;
        }
        if (te instanceof TileEntityElectrolyzer) {
            ...
            reader.setString(
                "waterTank",
                String.format("%s / %s mB", el.waterTank.getFluidAmount(), el.waterTank.getCapacity()));
            reader.setString(
                "gasTank1",
                String.format("%s / %s mB", el.liquidTank.getFluidAmount(), el.liquidTank.getCapacity()));
            reader.setString(
                "gasTank2",
                String.format("%s / %s mB", el.liquidTank2.getFluidAmount(), el.liquidTank2.getCapacity()));
            return CardState.OK;
        }
        if (te instanceof TileEntityMethaneSynthesizer) {
            ...
            reader.setString(
                "methaneTank",
                String.format("%s / %s mB", meth.liquidTank.getFluidAmount(), meth.liquidTank.getCapacity()));
            reader.setString(
                "gasTank1",
                String.format("%s / %s mB", meth.gasTank.getFluidAmount(), meth.gasTank.getCapacity()));
            reader.setString(
                "gasTank2",
                String.format("%s / %s mB", meth.gasTank2.getFluidAmount(), meth.gasTank2.getCapacity()));
            int counter = 0;
            ItemStack stack = ((IInventory) meth).getStackInSlot(2);
            if (stack != null && stack.getItem() instanceof ItemAtmosphericValve) counter = stack.stackSize;
            reader.setInt("valve", counter);
            counter = 0;
            stack = ((IInventory) meth).getStackInSlot(3);
            if (stack != null) counter = stack.stackSize;
        ...
        if (te instanceof TileEntityGasLiquefier) {
            ...
            reader.setString(
                "gasTank",
                String.format("%s / %s mB", gas.gasTank.getFluidAmount(), gas.gasTank.getCapacity()));
            FluidStack liquid = gas.gasTank.getFluid();
            if (liquid != null && liquid.getFluid() != null) reader.setString(
                "gasTankName",
                liquid.getFluid()
                    .getUnlocalizedName());
            else reader.setString("gasTankName", "");
            reader.setString(
                "liquidTank",
                String.format("%s / %s mB", gas.liquidTank.getFluidAmount(), gas.liquidTank.getCapacity()));
            liquid = gas.liquidTank.getFluid();
            if (liquid != null && liquid.getFluid() != null) reader.setString(
                "liquidTankName",
                liquid.getFluid()
                    .getUnlocalizedName());
            else reader.setString("liquidTankName", "");
            reader.setString(
                "liquidTank2",
                String.format("%s / %s mB", gas.liquidTank2.getFluidAmount(), gas.liquidTank2.getCapacity()));
            liquid = gas.liquidTank2.getFluid();
            if (liquid != null && liquid.getFluid() != null) reader.setString(
                "liquidTank2Name",
                liquid.getFluid()
                    .getUnlocalizedName());
            else reader.setString("liquidTank2Name", "");
            int counter = 0;
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        if (!reader.hasField("type")) return result;

        int type = reader.getInt("type");
        if (type == 3) {
            if (reader.getBoolean("active")) result.add(
                new PanelString(
                    "msg.ec.InfoPanelOxygenDetected",
                    EnumChatFormatting.GREEN + I18n.format("msg.ec.InfoPanelTrue"),
                    showLabels));
            else result.add(
                new PanelString(
                    "msg.ec.InfoPanelOxygenDetected",
                    EnumChatFormatting.RED + I18n.format("msg.ec.InfoPanelFalse"),
                    showLabels));
            return result;
        }
        if (type == 8) {
            result.add(
                new PanelString(
                    "msg.ec.InfoPanelOxygen",
                    String.format("%s / %s mB", reader.getDouble("oxygenStored"), reader.getDouble("oxygenCapacity")),
                    showLabels));
            return result;
        }
        ...
        if ((settings & 2) > 0) result.add(
            new PanelString(
                "msg.ec.InfoPanelEnergy",
                String.format(
                    "%s / %s gJ",
                    df.format(reader.getDouble("stored")),
                    df.format(reader.getDouble("capacity"))),
                showLabels));

        switch (type) {
            case 1:
                result
                    .add(new PanelString("msg.ec.InfoPanelCollecting", reader.getDouble("oxygenPerTick"), showLabels));
                result.add(
                    new PanelString(
                        "msg.ec.InfoPanelOxygen",
                        String
                            .format("%s / %s mB", reader.getDouble("oxygenStored"), reader.getDouble("oxygenCapacity")),
                        showLabels));
                break;
            case 2:
                result.add(new PanelString("msg.ec.InfoPanelOxygenUse", reader.getDouble("oxygenPerTick"), showLabels));
                result.add(
                    new PanelString("msg.ec.InfoPanelThermalControl", reader.getString("thermalStatus"), showLabels));
                result.add(
                    new PanelString(
                        "msg.ec.InfoPanelOxygen",
                        String
            ...
            case 6:
                ...
                break;
            case 7:
                if (!reader.getString("gasTankName")
                    .equals(""))
                    result.add(
                        new PanelString(
                            I18n.format(reader.getString("gasTankName")) + ": " + reader.getString("gasTank")));
                result.add(new PanelString("msg.ec.InfoPanelAtmosphericValve", reader.getInt("valve"), showLabels));
                if (!reader.getString("liquidTankName")
                    .equals(""))
                    result.add(
                        new PanelString(
                            I18n.format(reader.getString("liquidTankName")) + ": " + reader.getString("liquidTank")));
                if (!reader.getString("liquidTank2Name")
                    .equals(""))
                    result.add(
                        new PanelString(
                            I18n.format(reader.getString("liquidTank2Name")) + ": " + reader.getString("liquidTank2")));
                break;
            case 10:
                result.add(new PanelString("msg.ec.InfoPanelOutputgJ", reader.getInt("production"), showLabels));
                result
                    .add(new PanelString("msg.ec.InfoPanelEnvironmentalBoost", reader.getDouble("boost"), showLabels));
