...
public class CrossNuclearCraft extends CrossModBase {
    ...
    public NBTTagCompound getCardData(TileEntity te) {
        ...
        if (te instanceof TileMachineBase) {
            ...
            tag.setDouble(
                DataHelper.CONSUMPTION,
                active ? (int) Math.ceil(base.getEnergyRequired / base.getProcessTime) : 0);
            return tag;
        }
        if (te instanceof TileElectricFurnace) {
            TileElectricFurnace base = (TileElectricFurnace) te;
            ...
            tag.setDouble(
                DataHelper.CONSUMPTION,
                active ? (int) Math.ceil(base.getRequiredEnergy / base.getFurnaceSpeed) : 0);
            return tag;
        }
        if (te instanceof TileReactionGenerator) {
            TileReactionGenerator base = (TileReactionGenerator) te;
            tag.setDouble(DataHelper.ENERGY, base.energy);
            tag.setDouble(DataHelper.CAPACITY, base.getMaxEnergyStored(null));
            Boolean active = base.fuellevel >= base.requiredFuel
                && base.storage.getEnergyStored() <= base.storage.getMaxEnergyStored() - TileReactionGenerator.power
                && base.reactantlevel >= base.requiredReactant;
            tag.setBoolean(DataHelper.ACTIVE, active);
        ...
        if (te instanceof TileSolarPanel) {
            ...
            tag.setDouble(DataHelper.ENERGY, panel.storage.getEnergyStored());
            tag.setDouble(DataHelper.CAPACITY, panel.storage.getMaxEnergyStored());
            Boolean active = panel.storage.getEnergyStored() == 0 && te.getWorldObj()
                .canBlockSeeTheSky(te.xCoord, te.yCoord + 1, te.zCoord)
                && te.getWorldObj()
                    .getBlockLightValue(te.xCoord, te.yCoord + 1, te.zCoord) > 0;
            tag.setBoolean(DataHelper.ACTIVE, active);
            tag.setDouble(
                DataHelper.OUTPUT,
                active ? (int) Math.ceil(
                    1D * panel.power
                        * te.getWorldObj()
                            .getBlockLightValue(te.xCoord, te.yCoord + 1, te.zCoord)
                        * te.getWorldObj()
                            .getBlockLightValue(te.xCoord, te.yCoord + 1, te.zCoord)
                        / 225)
                    : 0);
        ...
        if (te instanceof TileStorage) {
            ...
            if (values != null) tag.setLong("diff", ((Long) values.get(0) - (Long) values.get(20)) / 20);
            return tag;
        }
        if (te instanceof TileFissionReactor) {
            ...
            tag.setInteger(
                "heatChange",
                active ? reactor.HReal : reactor.off == 1 && !reactor.flag && reactor.heat > 0 ? reactor.HCooling : 0);
        ...
        if (te instanceof TileFusionReactorBlock) {
            TileEntity core = te.getWorldObj()
                .getTileEntity(
                    te.xCoord + ((TileFusionReactorBlock) te).xOffset,
                    te.yCoord + ((TileFusionReactorBlock) te).yOffset,
                    te.zCoord + ((TileFusionReactorBlock) te).zOffset);
            if (core instanceof TileFusionReactor) {
                ...
                tag.setString(
                    "level1",
                    f1(reactor) + " "
                        + StatCollector.translateToLocal("gui.level1")
                        + ": "
                        + Math.round((level1(reactor) / 64000))
                        + "%");
                tag.setString(
                    "level2",
                    f2(reactor) + " "
                        + StatCollector.translateToLocal("gui.level1")
                        + ": "
                        + Math.round((level2(reactor) / 64000))
                        + "%");
        ...
        if (te instanceof TileFusionReactorSteamBlock) {
            TileEntity core = te.getWorldObj()
                .getTileEntity(
                    te.xCoord + ((TileFusionReactorSteamBlock) te).xOffset,
                    te.yCoord + ((TileFusionReactorSteamBlock) te).yOffset,
                    te.zCoord + ((TileFusionReactorSteamBlock) te).zOffset);
    ...
    public static String f1(TileFusionReactor reactor) {
        if (reactor.HLevel > 0) return "Hydrogen";
        if (reactor.DLevel > 0) return "Deuterium";
        if (reactor.TLevel > 0) return "Tritium";
        if (reactor.HeLevel > 0) return "Helium-3";
        if (reactor.BLevel > 0) return "Boron-11";
        if (reactor.Li6Level > 0) return "Lithium-6";
        if (reactor.Li7Level > 0) return "Lithium-7";
        return "Fuel";
    }

    public static String f2(TileFusionReactor reactor) {
        if (reactor.HLevel2 > 0) return "Hydrogen";
        if (reactor.DLevel2 > 0) return "Deuterium";
        if (reactor.TLevel2 > 0) return "Tritium";
        if (reactor.HeLevel2 > 0) return "Helium-3";
        if (reactor.BLevel2 > 0) return "Boron-11";
        if (reactor.Li6Level2 > 0) return "Lithium-6";
        if (reactor.Li7Level2 > 0) return "Lithium-7";
        return "Fuel";
    }

    public static int level1(TileFusionReactor reactor) {
        if (reactor.HLevel > 0) return reactor.HLevel;
        if (reactor.DLevel > 0) return reactor.DLevel;
        if (reactor.TLevel > 0) return reactor.TLevel;
        if (reactor.HeLevel > 0) return reactor.HeLevel;
        if (reactor.BLevel > 0) return reactor.BLevel;
        if (reactor.Li6Level > 0) return reactor.Li6Level;
        if (reactor.Li7Level > 0) return reactor.Li7Level;
        return 0;
    }

    public static int level2(TileFusionReactor reactor) {
        if (reactor.HLevel2 > 0) return reactor.HLevel2;
        if (reactor.DLevel2 > 0) return reactor.DLevel2;
        if (reactor.TLevel2 > 0) return reactor.TLevel2;
        if (reactor.HeLevel2 > 0) return reactor.HeLevel2;
        if (reactor.BLevel2 > 0) return reactor.BLevel2;
        if (reactor.Li6Level2 > 0) return reactor.Li6Level2;
        if (reactor.Li7Level2 > 0) return reactor.Li7Level2;
        return 0;
    }
    ...
    public ArrayList getHookValues(TileEntity te) {
        ...
        if (values == null) NuclearCraftHooks.map.put(te, null);
        return values;
    }
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_NUCLEARCRAFT,
            new Object[] { "IT", "PD", 'P', Items.paper, 'D', "dyeGreen", 'T',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'I', "ingotCopper" });
