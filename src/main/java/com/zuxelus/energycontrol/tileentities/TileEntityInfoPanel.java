...
public class TileEntityInfoPanel extends TileEntityInventory
    implements ITilePacketHandler, IScreenPart, ISlotItemFilter {
    ...
    public void setShowLabels(boolean newShowLabels) {
        if (!worldObj.isRemote && showLabels != newShowLabels) notifyBlockUpdate();
    ...
    public void setTickRate(int newValue) {
        if (!worldObj.isRemote && tickRate != newValue) notifyBlockUpdate();
        tickRate = newValue;
    }
    ...
    public void setColored(boolean newColored) {
        if (!worldObj.isRemote && colored != newColored) notifyBlockUpdate();
        colored = newColored;
    }
    ...
    public void setColorBackground(int c) {
        if (!worldObj.isRemote && colorBackground != c) notifyBlockUpdate();
        colorBackground = c;
    }
    ...
    public void setColorText(int c) {
        if (!worldObj.isRemote && colorText != c) notifyBlockUpdate();
        colorText = c;
    }
    ...
    public void setPowered(boolean value) {
        powered = value;
        if (!powered) removeTileEntityFromList();
    }

    public void setScreenData(NBTTagCompound nbtTagCompound) {
        screenData = nbtTagCompound;
        if (screen != null && worldObj.isRemote) screen.destroy(true, worldObj);
        if (screenData != null) {
            screen = EnergyControl.instance.screenManager.loadScreen(this);
            if (screen != null) screen.init(true, worldObj);
        }
    ...
    public void onServerMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                ...
                break;
            case 3:
                if (tag.hasKey("value")) setShowLabels(tag.getInteger("value") == 1);
                break;
            case 4:
            ...
            case 5:
                if (tag.hasKey("value")) setTickRate(tag.getInteger("value"));
                break;
            case 6:
                if (tag.hasKey("value")) setColorBackground(tag.getInteger("value"));
                break;
            case 7:
                if (tag.hasKey("value")) setColorText(tag.getInteger("value"));
                break;
        }
    }
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("tickRate")) tickRate = tag.getInteger("tickRate");
        if (tag.hasKey("showLabels")) showLabels = tag.getBoolean("showLabels");
        if (tag.hasKey("colorText")) colorText = tag.getInteger("colorText");
        if (tag.hasKey("colorBackground")) colorBackground = tag.getInteger("colorBackground");
        if (tag.hasKey("colored")) setColored(tag.getBoolean("colored"));

        if (tag.hasKey("screenData")) {
            if (worldObj != null) setScreenData((NBTTagCompound) tag.getTag("screenData"));
            else screenData = (NBTTagCompound) tag.getTag("screenData");
        } else screenData = null;
    ...
    public void invalidate() {
        if (!worldObj.isRemote) EnergyControl.instance.screenManager.unregisterScreenPart(this);
        super.invalidate();
    }
    ...
    public void updateEntityEnd() {
        ...
        if (!worldObj.isRemote) {
            ...
            updateTicker = tickRate - 1;
            if (hasCards()) markDirty();
        }
    }

    private boolean hasCards() {
        for (ItemStack card : getCards()) if (card != null) return true;
        return false;
    }
    ...
    public List<PanelString> getCardData(int settings, ItemStack cardStack, ItemCardReader reader, boolean isServer,
        boolean showLabels) {
    ...
    public List<PanelString> getPanelStringList(boolean isServer, boolean showLabels) {
        ...
        for (ItemStack card : cards) {
            ...
            if (state != CardState.OK && state != CardState.CUSTOM_ERROR) data = ItemCardReader.getStateMessage(state);
            else data = getCardData(settings, card, reader, isServer, showLabels);
            if (data == null) continue;
            joinedData.addAll(data);
            anyCardFound = true;
        }
        if (anyCardFound) return joinedData;
        return null;
    }
    ...
    public List<String> getPanelStringList(boolean isRaw) {
        ...
        if (joinedData == null || joinedData.size() == 0) return list;

        for (PanelString panelString : joinedData) {
            if (panelString.textLeft != null) list.add(formatString(panelString.textLeft, isRaw));
            if (panelString.textCenter != null) list.add(formatString(panelString.textCenter, isRaw));
            if (panelString.textRight != null) list.add(formatString(panelString.textRight, isRaw));
        }
        return list;
    }
    ...
    public boolean isColoredEval() {
        ...
        return stack != null && stack.getItem() instanceof ItemUpgrade
            && stack.getItemDamage() == ItemUpgrade.DAMAGE_COLOR;
    }
    ...
    public void markDirty() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (powered) {
                ...
                for (ItemStack card : getCards()) processCard(card, getCardSlot(card), itemStack);
            }
        }
    }
    ...
    public int getDisplaySettingsByCard(ItemStack card) {
        ...
        if (displaySettings.containsKey(slot)) {
            for (Map.Entry<String, Integer> entry : displaySettings.get(slot)
                .entrySet()) {
                if (StringUtils.getItemId(card)
                    .equals(entry.getKey())) return entry.getValue();
            }
        }
    ...
    public void setDisplaySettings(int slot, int settings) {
        ...
        if (!displaySettings.containsKey(slot)) displaySettings.put(slot, new HashMap<>());
        displaySettings.get(slot)
            .put(StringUtils.getItemId(stack), settings);
        if (!worldObj.isRemote) notifyBlockUpdate();
    }
    ...
    public void updateBlockState() {
        ...
        setPowered(flag);
        if (flag == (meta > 5)) return;

        if (flag) worldObj.scheduleBlockUpdate(xCoord, yCoord, zCoord, worldObj.getBlock(xCoord, yCoord, zCoord), 4);
        else {
    ...
    public void removeTileEntityFromList() {
        for (ItemStack card : getCards()) if (card != null) {
            ItemCardReader reader = new ItemCardReader(card);
    ...
    public void updateExtenders(Boolean active) {
        ...
        for (int x = screen.minX; x <= screen.maxX; x++)
            for (int y = screen.minY; y <= screen.maxY; y++) for (int z = screen.minZ; z <= screen.maxZ; z++) {
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (screen == null) return AxisAlignedBB.getBoundingBox(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D);
        return AxisAlignedBB
            .getBoundingBox(screen.minX, screen.minY, screen.minZ, screen.maxX + 1, screen.maxY + 1, screen.maxZ + 1);
    }
    ...
    public int findTexture() {
        ...
        if (scr != null) {
            switch (getFacingForge()) {
                case SOUTH:
                    return boolToInt(xCoord == scr.minX) + 2 * boolToInt(xCoord == scr.maxX)
                        + 4 * boolToInt(yCoord == scr.minY)
                        + 8 * boolToInt(yCoord == scr.maxY);
                case WEST:
                    return 8 * boolToInt(zCoord == scr.minZ) + 4 * boolToInt(zCoord == scr.maxZ)
                        + boolToInt(yCoord == scr.minY)
                        + 2 * boolToInt(yCoord == scr.maxY);
                case EAST:
                    return 8 * boolToInt(zCoord == scr.minZ) + 4 * boolToInt(zCoord == scr.maxZ)
                        + 2 * boolToInt(yCoord == scr.minY)
                        + boolToInt(yCoord == scr.maxY);
                case NORTH:
                    return boolToInt(xCoord == scr.minX) + 2 * boolToInt(xCoord == scr.maxX)
                        + 8 * boolToInt(yCoord == scr.minY)
                        + 4 * boolToInt(yCoord == scr.maxY);
                case UP:
                    return boolToInt(xCoord == scr.minX) + 2 * boolToInt(xCoord == scr.maxX)
                        + 8 * boolToInt(zCoord == scr.minZ)
                        + 4 * boolToInt(zCoord == scr.maxZ);
                case DOWN:
                    return boolToInt(xCoord == scr.minX) + 2 * boolToInt(xCoord == scr.maxX)
                        + 4 * boolToInt(zCoord == scr.minZ)
                        + 8 * boolToInt(zCoord == scr.maxZ);
            }
        }
        return 15;
    }
    ...
    public boolean isTouchCard(ItemStack stack) {
        return stack != null && stack.getItem() instanceof ITouchAction
            && ((ITouchAction) stack.getItem()).enableTouch(stack);
    }
    ...
    public boolean hasBars(ItemStack stack) {
        return stack != null && stack.getItem() instanceof IHasBars
            && ((IHasBars) stack.getItem()).enableBars(stack)
            && (getDisplaySettingsForCardInSlot(SLOT_CARD) & 1024) > 0;
    }
    ...
    public void renderImage(TextureManager manager, double displayWidth, double displayHeight) {
        ...
        if (isTouchCard()) ((ITouchAction) card).renderImage(manager, new ItemCardReader(stack));
        if (hasBars()) ((IHasBars) card).renderBars(manager, displayWidth, displayHeight, new ItemCardReader(stack));
    }

    protected void runTouchAction(TileEntityInfoPanel panel, ItemStack cardStack, ItemStack stack, int slot,
        boolean needsTouchUpgrade) {
    ...
    public void setFacing(short facing) {
        ...
        if ((facing == 0 || facing == 1) && rotation == ForgeDirection.DOWN) rotation = ForgeDirection.NORTH;
        if (facing != 0 && facing != 1 && rotation != ForgeDirection.DOWN) rotation = ForgeDirection.DOWN;
    }
