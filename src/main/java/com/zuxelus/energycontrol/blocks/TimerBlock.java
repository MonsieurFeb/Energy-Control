...
public class TimerBlock extends FacingBlockSmall {
    ...
    public int isProvidingWeakPower(IBlockAccess blockAccess, int x, int y, int z, int side) {
        ...
        if (!(te instanceof TileEntityTimer)) return 0;
        if (side == meta || side == ((TileEntityTimer) te).getRotation()
            .getOpposite()
            .ordinal()) return 0;
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        ...
        if (!world.isRemote) {
            ...
            if (be instanceof TileEntityTimer) ((TileEntityTimer) be).onNeighborChange();
        }
    }
}
