...
public class TileEntitySeedLibrary extends TileEntityInventory
    implements ITilePacketHandler, ISlotItemFilter, IEnergySink {
    ...
    public TileEntitySeedLibrary() {
        ...
        for (int i = 0; i < filters.length - 1; i++) filters[i] = new SeedLibraryFilter(null);
    ...
    public void setSliderValue(int slider, int value) {
        ...
        int arrow = slider % 2;
        if (bar == 0) {
            if (arrow == 0) filter.min_growth = value;
            else filter.max_growth = value;
        } else if (bar == 1) {
            if (arrow == 0) filter.min_gain = value;
            else filter.max_gain = value;
        } else if (bar == 2) {
            if (arrow == 0) filter.min_resistance = value;
            else filter.max_resistance = value;
        } else { // if (bar == 3)
            if (arrow == 0) filter.min_total = value * 3;
            else filter.max_total = value * 3;
        }
    ...
    private void actionPerformed(int button, boolean rightClick) {
        if (button == 0) importFromInventory();
        else if (button == 1) exportToInventory();
        else if (button == 2) {
            SeedLibraryFilter filter = getGUIFilter();
            ...
            if (rightClick) {
                ...
                markDirty();
            } else filters[6].copyFrom(filters[dir]);
        }
    }
    ...
    public void onClientMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                ...
                break;
            case 2:
                if (tag.hasKey("value")) seeds_available = tag.getInteger("value");
                break;
        }
    }
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < 7; i++) filters[i].loadFromNBT(filterList.getCompoundTagAt(i));
    ...
    public void updateEntity() {
        ...
        if (stack != null && energy < CAPACITY && stack.getItem() instanceof IElectricItem) {
            ...
            if (old == 0 && energy >= CONSUMPTION) updateState();
        }
        if (energy >= CONSUMPTION) energy -= CONSUMPTION;
        else {
            energy = 0;
            updateState();
        }
        ...
        switch (index) {
            case SLOT_DISCHARGER:
                return stack.getItem() instanceof IElectricItem
                    && ((IElectricItem) stack.getItem()).getTier(stack) <= TIER;
            case FAKE_SLOT:
                return false;
            default:
    ...
    private void loadSeed(ItemStack seed) {
        ...
        if (stored != null && stored != null) {
            ...
            for (SeedLibraryFilter filter : filters) filter.newSeed(stored);
        }
    }

    private void updateCountIfMatch(ItemStack seed) {
        SeedLibraryFilter filter = getGUIFilter();
        if (!filter.bulk_mode && filter.isMatch(seed)) updateSeedCount();
    }
    ...
    public void updateSeedCount() {
        ...
        if (FMLCommonHandler.instance()
            .getEffectiveSide()
            .isClient()) // world is empty
            return;

        // Notify all nearby players that the seed count has changed.
        if (seeds_available > 65535) seeds_available = 65535;
    ...
    private void unloadSeed(ItemStack seed) {
        ...
        if (stored.stackSize <= seed.stackSize) {
            ...
            for (SeedLibraryFilter filter : filters) filter.lostSeed(stored);
            stored.stackSize -= seed.stackSize;
        } else {
    ...
    public void importFromInventory() {
        ...
        for (int i = 1; i < 9; i++) {
            ...
            if (stack != null && stack.getItem() == IC2Items.getItem("cropSeed")
                .getItem()) {
                loadSeed(stack);
                inventory[i] = null;
            }
    ...
    public void exportToInventory() {
        getGUIFilter().bulk_mode = true;
        for (int i = 1; i < 9; i++) if (getStackInSlot(i) == null) {
            // Get a seed from the active filter.
            ItemStack seed = filters[6].getSeed(deepContents.values());
            if (seed == null || seed.stackSize == 0) break; // No seeds left; stop exporting.
    ...
    public double injectEnergy(ForgeDirection directionFrom, double amount, double voltage) {
        ...
        if (energy >= CONSUMPTION && old < CONSUMPTION && !worldObj.isRemote) updateState();
        return left;
    }
