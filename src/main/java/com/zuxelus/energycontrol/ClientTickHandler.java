    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (altPressed != alt) {
            ...
            if (Minecraft.getMinecraft()
                .getNetHandler() != null) ChannelHandler.updateSeverKeys(alt);
        }
    }

    @SubscribeEvent
    public void render(RenderWorldLastEvent event) {
        if (TileEntityRendererDispatcher.instance.field_147553_e == null) return;
        // TileEntityRendererDispatcher.instance.preDrawBatch();
        for (TileEntity te : holo_panels) TileEntityRendererDispatcher.instance.renderTileEntity(te, -1);
