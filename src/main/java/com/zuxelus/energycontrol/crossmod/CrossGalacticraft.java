...
public class CrossGalacticraft extends CrossModBase {
    ...
    public NBTTagCompound getEnergyData(TileEntity te) {
        if (te instanceof TileEntityAFSU) return null;
    ...
    public List<FluidInfo> getAllTanks(TileEntity te) {
        if (te instanceof IFluidHandler) {
            ...
            for (FluidTankInfo tank : info) result.add(new FluidInfo(tank.fluid, tank.capacity));
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_GALACTICRAFT,
            new Object[] { "RF", "PB", 'P', Items.paper, 'R', "dustRedstone", 'F',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'B',
