...
public class ItemCardBigReactors extends ItemCardBase {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        NBTTagCompound tag = CrossModLoader.getCrossMod(ModIDs.BIG_REACTORS)
            .getCardData(te);
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        if (reader.hasField("cooling") && (settings & 64) > 0) result.add(
            new PanelString(
                "msg.ec.InfoPanelPassiveCooling",
                reader.getBoolean("cooling")
                    .toString(),
                showLabels));
        if (reader.hasField(DataHelper.ENERGY) && (settings & 2) > 0) result
            .add(new PanelString("msg.ec.InfoPanelEnergy", reader.getDouble(DataHelper.ENERGY), "RF", showLabels));
        if (reader.hasField(DataHelper.CAPACITY) && (settings & 2) > 0) result
            .add(new PanelString("msg.ec.InfoPanelCapacity", reader.getDouble(DataHelper.CAPACITY), "RF", showLabels));
        ...
        if ((settings & 64) > 0) {
            if (reader.hasField("fuelCapacity")) result
                .add(new PanelString("msg.ec.InfoPanelCapacity", reader.getInt("fuelCapacity"), "mB", showLabels));
            if (reader.hasField("consumption")) result
                .add(new PanelString("msg.ec.InfoPanelBurnupRatemb", reader.getDouble("consumption"), showLabels));
            ...
            if (reader.hasField("efficiency")) result
                .add(new PanelString("msg.ec.InfoPanelRotorEfficiency", reader.getDouble("efficiency"), showLabels));
