...
public class CrossIC2Exp extends CrossModBase {
    ...
    public NBTTagCompound getEnergyData(TileEntity te) {
        ...
        if (te instanceof TileEntitySemifluidGenerator) {
            ...
            tag.setDouble(
                DataHelper.CAPACITY,
                DataHelper.getShort(TileEntitySemifluidGenerator.class, "maxStorage", te));
            return tag;
        }
    ...
    public List<FluidInfo> getAllTanks(TileEntity te) {
        ...
        if (te instanceof IFluidHandler) {
            ...
            for (FluidTankInfo tank : list) result.add(new FluidInfo(tank.fluid, tank.capacity));
            return result;
        }
    ...
    public NBTTagCompound getCardData(TileEntity te) {
        try {
            ...
            if (te instanceof TileEntityBaseGenerator) {
                ...
                if (te instanceof TileEntityRTGenerator) {
                    ...
                    for (int i = 0; i < ((TileEntityRTGenerator) te).fuelSlot.size(); i++)
                        if (((TileEntityRTGenerator) te).fuelSlot.get(i) != null) counter++;
                    tag.setInteger("pellets", counter);
                    if (counter == 0
                        || ((TileEntityBaseGenerator) te).storage >= ((TileEntityBaseGenerator) te).maxStorage) {
                ...
                if (te instanceof TileEntitySolarGenerator) {
                    ...
                    boolean active = ((TileEntitySolarGenerator) te).sunIsVisible
                        && ((TileEntityBaseGenerator) te).storage < ((TileEntityBaseGenerator) te).maxStorage;
                    tag.setDouble(DataHelper.MULTIPLIER, multiplier);
                    tag.setBoolean(DataHelper.ACTIVE, active);
                    tag.setDouble(
                        DataHelper.OUTPUT,
                        active ? multiplier * (te.getWorldObj()
                            .getBlockLightValue(te.xCoord, 255, te.zCoord) / 15.0F) : 0);
                    return tag;
                }
                if (te instanceof TileEntityWaterGenerator) {
                    boolean active = (((TileEntityWaterGenerator) te).water > 0
                        || ((TileEntityWaterGenerator) te).fuel > 0) && ((TileEntityBaseGenerator) te).storage < 4;
                    ...
                    else tag.setDouble(DataHelper.OUTPUT, 0);
                    return tag;
                }
                if (te instanceof TileEntityWindGenerator) {
                    boolean active = ((TileEntityWindGenerator) te).subproduction > 0
                        && ((TileEntityBaseGenerator) te).storage < 4;
            ...
            if (te instanceof TileEntitySemifluidGenerator) {
                ...
                tag.setDouble(
                    DataHelper.OUTPUT,
                    DataHelper.getDouble(TileEntitySemifluidGenerator.class, "production", te));
            ...
            if (te instanceof TileEntityKineticGenerator) {
                ...
                tag.setDouble(
                    DataHelper.CAPACITY,
                    DataHelper.getInt(TileEntityKineticGenerator.class, "maxEUStorage", te));
                tag.setDouble(DataHelper.OUTPUT, ((TileEntityKineticGenerator) te).getproduction());
                tag.setDouble(
                    DataHelper.MULTIPLIER,
                    DataHelper.getDouble(TileEntityKineticGenerator.class, "productionpeerkineticunit", te));
                return tag;
            }
            ...
            if (te instanceof TileEntityElectricKineticGenerator) {
                ...
                int counter = 0;
                for (int i = 0; i < generator.slotMotor.size(); i++) if (generator.slotMotor.get(i) != null) counter++;
                ...
                ArrayList values = getHookValues(te);
                if (values != null) tag.setDouble(DataHelper.OUTPUTKU, (double) values.get(0));
                Energy energy = (Energy) generator.getComponent("energy");
                tag.setDouble(DataHelper.ENERGY, energy.getEnergy());
                tag.setDouble(DataHelper.CAPACITY, energy.getCapacity());
                tag.setDouble(
                    DataHelper.MULTIPLIER,
                    DataHelper.getFloat(TileEntityElectricKineticGenerator.class, "kuPerEU", te));
                return tag;
            }
            ...
            if (te instanceof TileEntityStirlingKineticGenerator) {
                ...
                tag.setDouble(
                    DataHelper.ENERGYKU,
                    DataHelper.getInt(TileEntityStirlingKineticGenerator.class, "kUBuffer", te));
                tag.setDouble(
                    DataHelper.CAPACITYKU,
                    DataHelper.getInt(TileEntityStirlingKineticGenerator.class, "maxkUBuffer", te));
                tag.setDouble(
                    DataHelper.ENERGYHU,
                    DataHelper.getInt(TileEntityStirlingKineticGenerator.class, "heatbuffer", te));
                tag.setDouble(
                    DataHelper.CAPACITYHU,
                    DataHelper.getInt(TileEntityStirlingKineticGenerator.class, "maxHeatbuffer", te));
            ...
            if (te instanceof TileEntityWaterKineticGenerator) {
                ...
                tag.setDouble(
                    DataHelper.MULTIPLIER,
                    DataHelper.getFloat(TileEntityWaterKineticGenerator.class, "outputModifier", te));
                tag.setInteger("height", generator.yCoord);
                if (generator.rotorSlot.isEmpty()) tag.setDouble("health", -1);
                else tag.setDouble(
                    "health",
                    (double) (100.0F - generator.rotorSlot.get()
                        .getItemDamage() * 100.0F
                        / generator.rotorSlot.get()
                            .getMaxDamage()));
                return tag;
            }
            if (te instanceof TileEntityWindKineticGenerator) {
                ...
                tag.setDouble(
                    DataHelper.MULTIPLIER,
                    generator.getefficiency() * TileEntityWindKineticGenerator.outputModifier);
                tag.setInteger("height", generator.yCoord);
                if (generator.rotorSlot.isEmpty()) tag.setDouble("health", -1);
                else tag.setDouble(
                    "health",
            ...
            if (te instanceof TileEntityHeatSourceInventory) {
                ...
                if (te instanceof TileEntityElectricHeatGenerator) {
                    ...
                    for (int i = 0; i < ((TileEntityElectricHeatGenerator) te).CoilSlot.size(); i++)
                        if (((TileEntityElectricHeatGenerator) te).CoilSlot.get(i) != null) counter++;
                    ...
                    tag.setInteger(
                        DataHelper.OUTPUTHU,
                        active ? ((TileEntityHeatSourceInventory) te).gettransmitHeat() : 0);
                    return tag;
                }
                ...
                if (te instanceof TileEntityLiquidHeatExchanger) {
                    ...
                    for (int i = 0; i < ((TileEntityLiquidHeatExchanger) te).heatexchangerslots.size(); i++)
                        if (((TileEntityLiquidHeatExchanger) te).heatexchangerslots.get(i) != null) counter++;
                ...
                if (te instanceof TileEntityRTHeatGenerator) {
                    ...
                    for (int i = 0; i < ((TileEntityRTHeatGenerator) te).fuelSlot.size(); i++)
                        if (((TileEntityRTHeatGenerator) te).fuelSlot.get(i) != null) counter++;
                    tag.setInteger("pellets", counter);
                ...
                if (te instanceof TileEntitySolidHeatGenerator) {
                    ...
                    tag.setInteger(
                        "energy",
                        DataHelper.getInt(TileEntitySolidHeatGenerator.class, "heatbuffer", te)
                            + ((TileEntitySolidHeatGenerator) te).getHeatBuffer());
                    return tag;
                }
            }
            ...
            if (te instanceof TileEntityElectricMachine) {
                ...
                if (te instanceof TileEntityCondenser) {
                    ...
                    tag.setDouble(
                        "progress",
                        ((TileEntityCondenser) te).progress * 100.0D / ((TileEntityCondenser) te).maxprogress);
                ...
                if (te instanceof TileEntityFluidRegulator) {
                    tag.setDouble(
                        DataHelper.OUTPUTMB,
                        DataHelper.getInt(TileEntityFluidRegulator.class, "mode", te) == 0
                            ? ((TileEntityFluidRegulator) te).getoutputmb() / 20.0D
                            : ((TileEntityFluidRegulator) te).getoutputmb());
            ...
            if (te instanceof TileEntityReactorAccessHatch) return getReactorData(
                ((TileEntityReactorChamberElectric) ((TileEntityReactorAccessHatch) te).getReactor()).getReactor());
        } catch (Throwable t) {}
        return null;
    }

    private boolean isActive(TileEntity te) {
        if (te instanceof TileEntityGeoGenerator || te instanceof TileEntityStirlingGenerator
            || te instanceof TileEntitySolarGenerator) return ((TileEntityBlock) te).getActive();
        if (te instanceof TileEntityBaseGenerator) return ((TileEntityBaseGenerator) te).isConverting();
        return false;
    }
    ...
    private NBTTagCompound getReactorData(TileEntityNuclearReactorElectric reactor) {
        ...
        if (reactor.isFluidCooled()) tag.setDouble("outputmB", reactor.EmitHeat);
        else tag.setDouble(DataHelper.OUTPUT, reactor.getReactorEUEnergyOutput());
        ...
        for (int i = 0; i < slotCount; i++) {
            ...
            if (stack != null) dmgLeft = Math.max(dmgLeft, getNuclearCellTimeLeft(stack));
        }
    ...
    private static int getNuclearCellTimeLeft(ItemStack stack) {
        ...
        if (item instanceof ItemReactorUranium || item instanceof ItemReactorLithiumCell
            ...
            return ((ICustomDamageItem) item).getMaxCustomDamage(stack)
                - ((ICustomDamageItem) item).getCustomDamage(stack);
        return 0;
    }
    ...
    public int getHeat(World world, int x, int y, int z) {
        ...
        if (reactor != null) return reactor.getHeat();
    ...
    public ArrayList getHookValues(TileEntity te) {
        ...
        if (values == null) IC2Hooks.map.put(te, null);
        return values;
    }
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.blockAfsu,
            new Object[] { "MGM", "IAI", "MGM", 'I', IC2Items.getItem("iridiumPlate"), 'G',
                IC2Items.getItem("glassFiberCableItem"), 'M', IC2Items.getItem("mfsUnit"), 'A',
                new ItemStack(ModItems.itemAFB, 1, Short.MAX_VALUE) });

        Recipes.addShapedRecipe(
            ModItems.blockSeedAnalyzer,
            new Object[] { " C ", "WAW", "WBW", 'C', IC2Items.getItem("cropnalyzer"), 'W', "plankWood", 'A',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.MACHINE_CASING), 'B', "circuitBasic" });

        Recipes.addShapedRecipe(
            ModItems.blockSeedLibrary,
            new Object[] { "GGG", "CAC", "CSC", 'G', "blockGlass", 'C', Blocks.chest, 'A', "circuitAdvanced", 'S',
                ModItems.blockSeedAnalyzer });

        Recipes.addShapedRecipe(
            ModItems.itemThermometerDigital,
            32767,
            new Object[] { "RI ", "ITI", " IP", 'R', "itemRubber", 'T', ModItems.itemThermometer, 'I', "plateIron", 'P',
                IC2Items.getItem("powerunitsmall") });

        Recipes.addShapedRecipe(
            ModItems.itemAFB,
            new Object[] { "GIG", "IUI", "GIG", 'G', IC2Items.getItem("glassFiberCableItem"), 'I',
                IC2Items.getItem("iridiumPlate"), 'U', IC2Items.getItem("uuMatterCell") });

        Recipes.addShapedRecipe(
            ModItems.itemAFSUUpgradeKit,
            new Object[] { "MGM", "IAI", "MGM", 'I', IC2Items.getItem("iridiumPlate"), 'G',
                IC2Items.getItem("glassFiberCableItem"), 'M', IC2Items.getItem("mfsUnit"), 'A',
                new ItemStack(ModItems.itemAFB, 1, Short.MAX_VALUE) });
        ...
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_IC2,
            new Object[] { "CF", "PL", 'P', Items.paper, 'C', IC2Items.getItem("advIronIngot"), 'F',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'L', "dyeLightGray" });
