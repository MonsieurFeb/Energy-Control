...
public class GuiButtonGeneral extends GuiButton {
    ...
    public GuiButtonGeneral(int id, int left, int top, int width, int height, ResourceLocation texture, int textureLeft,
        int textureTop) {
        this(id, left, top, width, height, "", texture, textureLeft, textureTop, 0, "");
    }

    public GuiButtonGeneral(int id, int left, int top, int width, int height, ResourceLocation texture, int textureLeft,
        int textureTop, int textureTopOff) {
    ...
    public GuiButtonGeneral(int id, int left, int top, int width, int height, String text, ResourceLocation texture,
        int textureLeft, int textureTop, int textureTopOff, String tooltip) {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        ...
        if (texture != null) mc.getTextureManager()
            .bindTexture(texture);
        ...
        if (texture != null) func_146110_a(
            xPosition,
            yPosition,
            textureLeft / scale,
            field_146123_n ? (textureTop + textureTopOff) / scale : textureTop / scale,
            width,
            height,
            256 / scale,
            256 / scale);
        ...
        if (!displayString.equals("")) {
            ...
            fontRenderer.drawString(
                displayString,
                xPosition + (width - fontRenderer.getStringWidth(displayString)) / 2,
                yPosition + (height - 7) / 2,
                enabled ? field_146123_n ? hoverColor : color : -0x5F5F60);
        }
    }
