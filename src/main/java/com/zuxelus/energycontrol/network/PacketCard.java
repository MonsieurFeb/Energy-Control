...
public class PacketCard implements IMessage, IMessageHandler<PacketCard, IMessage> {
    ...
    public IMessage onMessage(PacketCard message, MessageContext ctx) {
        ...
        if (ctx.side == Side.CLIENT) te = FMLClientHandler.instance()
        ...
        if (!stack.getItem()
            .getClass()
            .getName()
            .equals(message.className)) {
            EnergyControl.logger.warn(
                "Class mismatch: '{}' != '{}'",
                message.className,
                stack.getItem()
                    .getClass()
        ...
        if (ctx.side == Side.CLIENT) panel.resetCardData();
        return null;
    }
}
