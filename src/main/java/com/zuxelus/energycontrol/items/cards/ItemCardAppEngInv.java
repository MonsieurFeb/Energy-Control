...
public class ItemCardAppEngInv extends ItemCardBase implements ITouchAction {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        if (stacks.size() < 1) return CardState.OK;
        ...
        if (te instanceof TileCableBus) {
            ...
            if (cb != null) for (ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (part instanceof PartStorageMonitor) {
                    ...
                    if (proxy != null && proxy.getNode() != null
                        && proxy.getNode()
                            .getGrid() != null) {
                        gridList = proxy.getNode()
                            .getGrid()
                            .getNodes();
                        if (gridList != null) for (IGridNode node : gridList) {
                            IGridHost host = node.getMachine();
                            if (host instanceof TileChest) {
                                ItemStack stack = ((TileChest) host).getInternalInventory()
                                    .getStackInSlot(1);
                                updateValues(stack, stacks);
                            } else if (host instanceof TileDrive) {
                                for (int i = 0; i < ((TileDrive) host).getInternalInventory()
                                    .getSizeInventory(); i++) {
                                    ItemStack stack = ((TileDrive) host).getInternalInventory()
                                        .getStackInSlot(i);
                                    updateValues(stack, stacks);
                                }
                            }
    ...
    private void updateValues(ItemStack cell, ArrayList<ItemStack> stacks) {
        ...
        for (StorageChannel channel : StorageChannel.values()) {
            IMEInventory handler = AEApi.instance()
                .registries()
                .cell()
            ...
            if (handler instanceof IMEInventoryHandler) {
                ...
                for (IAEStack st : monitor.getStorageList()) if (st instanceof IAEItemStack)
                    for (ItemStack stack : stacks) if (stack.isItemEqual(((IAEItemStack) st).getItemStack()))
    ...
    public boolean runTouchAction(World world, ICardReader reader, ItemStack current) {
        ...
        ArrayList<ItemStack> stacks = reader.getItemStackList(true);
        for (ItemStack stack : stacks) if (stack.isItemEqual(current)) return false;
