...
public class ItemCardHBM extends ItemCardBase {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        NBTTagCompound tag = CrossModLoader.getCrossMod(ModIDs.HBM)
            .getCardData(te);
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        if (reader.hasField(DataHelper.ENERGYTU)) result
            .add(new PanelString("msg.ec.InfoPanelEnergy", reader.getLong(DataHelper.ENERGYTU), "TU", showLabels));
        if (reader.hasField("energy_"))
            result.add(new PanelString("msg.ec.InfoPanelEnergy", reader.getLong("energy_"), showLabels));
        if (reader.hasField(DataHelper.CAPACITY)) result
            .add(new PanelString("msg.ec.InfoPanelCapacity", reader.getLong(DataHelper.CAPACITY), "HE", showLabels));
        if (reader.hasField(DataHelper.CAPACITYTU)) result
            .add(new PanelString("msg.ec.InfoPanelCapacity", reader.getLong(DataHelper.CAPACITYTU), "TU", showLabels));
        if (reader.hasField("capacity_"))
            result.add(new PanelString("msg.ec.InfoPanelCapacity", reader.getLong("capacity_"), showLabels));
        if (reader.hasField("consumptionHE")) result
            .add(new PanelString("msg.ec.InfoPanelConsumption", reader.getDouble("consumptionHE"), "HE/t", showLabels));
        if (reader.hasField(DataHelper.CONSUMPTION)) result.add(
            new PanelString(
                "msg.ec.InfoPanelConsumption",
                reader.getDouble(DataHelper.CONSUMPTION),
                "mB/t",
                showLabels));
        if (reader.hasField("consumption_")) result.add(
            new PanelString("msg.ec.InfoPanelConsumption", reader.getString("consumption_") + " mB/t", showLabels));
        if (reader.hasField(DataHelper.OUTPUT)) result
            .add(new PanelString("msg.ec.InfoPanelOutput", reader.getDouble(DataHelper.OUTPUT), "HE/t", showLabels));
        if (reader.hasField(DataHelper.OUTPUTMB)) result
            .add(new PanelString("msg.ec.InfoPanelOutput", reader.getDouble(DataHelper.OUTPUTMB), "mB/t", showLabels));
        if (reader.hasField(DataHelper.OUTPUTTU)) result
            .add(new PanelString("msg.ec.InfoPanelOutput", reader.getDouble(DataHelper.OUTPUTTU), "TU/t", showLabels));
        if (reader.hasField(DataHelper.DIFF)) result
            .add(new PanelString("msg.ec.InfoPanelDifference", reader.getLong(DataHelper.DIFF), "HE/t", showLabels));
        ...
        if (reader.hasField("chunkRad")) if (showLabels) result.add(
            new PanelString(
                StatCollector.translateToLocalFormatted("geiger.chunkRad") + " " + reader.getString("chunkRad")));
        else result.add(new PanelString(reader.getString("chunkRad")));
