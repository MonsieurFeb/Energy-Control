...
public class CrossModLoader {
    ...
    public static void preInit() {
        CROSS_MODS.put(
            ModIDs.IC2,
            Loader.isModLoaded(ModIDs.IC2_CLASSIC) ? new CrossModBase()
                /* CrossIC2Classic */ : Loader.isModLoaded(ModIDs.IC2) ? new CrossIC2Exp() : new CrossModBase());
        ...
        if (Loader.isModLoaded(ModIDs.GREGTECH)) {
            ...
            if (container != null) {
                ...
                if (mod != null) {
                    if (mod.getClass()
                        .getName()
                        .equals("gregtech.GT6_Main")) loadCrossMod(ModIDs.GREGTECH, CrossGregTech6::new);
                    if (mod.getClass()
                        .getName()
                        .equals("gregtech.GT_Mod")) loadCrossMod(ModIDs.GREGTECH, CrossGregTech5u::new);
                }
            }
        }
        ...
        if (Loader.isModLoaded(ModIDs.HBM)) {
            ...
            Matcher matcher = Pattern.compile("\\d\\d\\d\\d")
                .matcher(container.getVersion());
            if (matcher.find()) {
                int version = Integer.parseInt(
                    container.getVersion()
                        .substring(matcher.start(), matcher.end()));
                if (version >= 4977) loadCrossModSafely(ModIDs.HBM, () -> CrossHBM::new);
            }
        }
    ...
    private static void loadCrossModSafely(String modid, Supplier<Supplier<? extends CrossModBase>> factory) {
        CROSS_MODS.put(
            modid,
            Loader.isModLoaded(modid) ? factory.get()
                .get() : new CrossModBase());
    }
    ...
    public static boolean isElectricItem(ItemStack stack) {
        ...
        for (CrossModBase crossMod : CROSS_MODS.values()) if (crossMod.isElectricItem(stack)) return true;
        return false;
    }

    public static double dischargeItem(ItemStack stack, double amount) {
        for (CrossModBase crossMod : CROSS_MODS.values()) if (crossMod.isElectricItem(stack)) {
    ...
    public static void registerItems() {
        for (CrossModBase crossMod : CROSS_MODS.values()) crossMod.registerItems();
    }

    public static void registerTileEntities() {
        for (CrossModBase crossMod : CROSS_MODS.values()) crossMod.registerTileEntities();
    }

    public static void loadRecipes() {
        for (CrossModBase crossMod : CROSS_MODS.values()) crossMod.loadRecipes();
    }

    public static void removeTileEntity(TileEntity te) {
        if (te == null) return;
        for (CrossModBase crossMod : CROSS_MODS.values()) crossMod.removeTileEntity(te);
    }
}
