...
public class BlockRenderer implements ISimpleBlockRenderingHandler { // 1.7.10
    ...
    public void renderInventoryBlock(Block block, int meta, int model, RenderBlocks renderer) {
        ...
        if (block instanceof InfoPanel || block instanceof InfoPanelExtender
            || block instanceof KitAssembler
            || block instanceof RemoteThermalMonitor
            || block instanceof AverageCounter
            || block instanceof EnergyCounter
            || block instanceof SeedAnalyzer
            || block instanceof SeedLibrary
            || block instanceof RangeTrigger
            || block instanceof AFSU) meta = 2;
        if (block instanceof HoloPanel || block instanceof HoloPanelExtender) meta = 5;
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int model,
        ...
        if ((!(renderer.overrideBlockTexture != null && renderer.overrideBlockTexture.getIconName()
            .indexOf("destroy_stage") > -1))
            && (block instanceof TimerBlock || block instanceof InfoPanel
                || block instanceof InfoPanelExtender
