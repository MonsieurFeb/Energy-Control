...
public class ItemKitMain extends Item implements IItemKit {
    ...
    public static void register(Supplier<ItemKitBase> factory) {
        ...
        if (checkKit(item)) KITS.put(item.getDamage(), item);
    }

    private static boolean checkKit(ItemKitBase item) {
        if (!KITS.containsKey(item.getDamage())) return true;
        if (item.getDamage() <= ItemCardType.KIT_MAX) EnergyControl.logger.warn(
            String.format(
                "Kit %s was not registered. ID %d is already used for standard kit.",
                item.getUnlocalizedName(),
                item.getDamage()));
        else EnergyControl.logger.warn(
            String.format(
                "Kit %s was not registered. ID %d is already used for extended kit.",
                item.getUnlocalizedName(),
                item.getDamage()));
        return false;
    }
    ...
    public String getUnlocalizedName(ItemStack stack) {
        ...
        if (KITS.containsKey(damage)) return KITS.get(damage)
            .getUnlocalizedName();
        return "";
    }
    ...
    public ItemStack getSensorCard(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side) {
        int damage = stack.getItemDamage();
        if (KITS.containsKey(damage)) return KITS.get(damage)
            .getSensorCard(stack, player, world, x, y, z, side);
        return null;
    }
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (stack == null || player == null || !(player instanceof EntityPlayerMP)) return false;
