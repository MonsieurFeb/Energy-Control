...
public class GuiTimer extends GuiContainerBase {
    ...
    public void initGui() {
        ...
        buttonList.add(
            new CompactButton(
                ...
                70,
                12,
                timer.getInvertRedstone() ? "No Redstone" : "Redstone"));
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (textboxTimer != null) textboxTimer.drawTextBox();
    }
    ...
    public void updateScreen() {
        ...
        if (textboxTimer != null) {
            ...
            if (isWorking) textboxTimer.setText(timer.getTimeString());
        }
    }
    ...
    private void updateTime(int delta) {
        if (textboxTimer == null) return;
        int time = 0;
        try {
            String value = textboxTimer.getText();
            if (timer.getIsTicks()) {
                if (!"".equals(value)) time = Integer.parseInt(value);
            } else {
                String[] times = value.split(":");
                if (times.length == 2) time = (Integer.parseInt(times[0]) * 60 + Integer.parseInt(times[1])) * 20;
                if (times.length == 3) time = (Integer.parseInt(times[0]) * 3600 + Integer.parseInt(times[1]) * 60
                    + Integer.parseInt(times[2])) * 20;
            }
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        switch (button.id) {
            ...
            case 7:
                ...
                NetworkHelper
                    .updateSeverTileEntity(timer.xCoord, timer.yCoord, timer.zCoord, 2, invertRedstone ? 0 : 1);
    ...
    protected void keyTyped(char typedChar, int keyCode) {
        if (keyCode == 1) // Esc button
            ...
            updateTime(0);
        else if (textboxTimer != null && textboxTimer.isFocused()
            && (Character.isDigit(typedChar) || typedChar == 0 || typedChar == 8))
