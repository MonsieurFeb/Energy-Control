...
public class CrossHBM extends CrossModBase {
    ...
    public boolean isElectricItem(ItemStack stack) {
        if (stack.getItem() instanceof ItemBattery) return true;
        return false;
    }
    ...
    public NBTTagCompound getEnergyData(TileEntity te) {
        if (te instanceof IInfoProviderEC) {
            ...
            if (tag.hasKey(DataHelper.ENERGY)) result.setDouble(DataHelper.ENERGY, tag.getDouble(DataHelper.ENERGY));
    ...
    public int getHeat(World world, int x, int y, int z) {
        ...
        for (int xoffset = -3; xoffset < 4; xoffset++)
            for (int yoffset = -1; yoffset < 2; yoffset++) for (int zoffset = -3; zoffset < 4; zoffset++) {
    ...
    private int getHeat(TileEntity te) {
        if (te instanceof TileEntityRBMKBase) return (int) ((TileEntityRBMKBase) te).heat;
        return -1;
    }
    ...
    public List<FluidInfo> getAllTanks(TileEntity te) {
        ...
        if (te instanceof TileEntityDummy) {
            te = te.getWorldObj()
                .getTileEntity(
                    ((TileEntityDummy) te).targetX,
                    ((TileEntityDummy) te).targetY,
                    ((TileEntityDummy) te).targetZ);
        }
        List<FluidInfo> result = new ArrayList<>();
        if (te instanceof IFluidUser) {
            FluidTank[] list = ((IFluidUser) te).getAllTanks();
            if (list.length == 0) return null;
            for (FluidTank tank : list) result.add(toFluidInfo(tank));
            return result;
        }
        return null;
    }
    ...
    private void addTank(String name, NBTTagCompound tag, FluidTank tank) {
        if (tank.getFill() == 0) tag.setString(name, "N/A");
        else {
            String fluidName = tank.getTankType()
                .getUnlocalizedName();
            if (!StatCollector.translateToLocal(fluidName)
                .equals(fluidName))
                tag.setString(
                    name,
                    String.format("%s: %s mB", StatCollector.translateToLocal(fluidName), tank.getFill()));
            else tag.setString(
                name,
                String.format(
                    "%s: %s mB",
    ...
    public NBTTagCompound getCardData(TileEntity te) {
        ...
        if (te instanceof TileEntityMachineBattery) {
            ...
            if (values != null) tag.setLong(DataHelper.DIFF, ((Long) values.get(0) - (Long) values.get(20)) / 20);
            return tag;
        }
        if (te instanceof IInfoProviderEC) {
            NBTTagCompound tag = new NBTTagCompound();
    ...
    public ArrayList getHookValues(TileEntity te) {
        ...
        if (values == null) HBMHooks.map.put(te, null);
        return values;
    }
    ...
    public void loadRecipes() {
        Recipes.addShapedRecipe(
            ModItems.itemKit,
            ItemCardType.KIT_HBM,
            new Object[] { "IT", "PD", 'P', Items.paper, 'D', "dyeBlack", 'T',
                new ItemStack(ModItems.itemComponent, 1, ItemComponent.RADIO_TRANSMITTER), 'I',
                com.hbm.items.ModItems.ingot_steel });
