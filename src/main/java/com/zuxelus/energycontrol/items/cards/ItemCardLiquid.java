...
public class ItemCardLiquid extends ItemCardBase implements IHasBars {
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        if ((settings & 1) > 0) {
            ...
            if (name.isEmpty()) name = isServer ? "N/A" : I18n.format("msg.ec.None");
            result.add(new PanelString("msg.ec.InfoPanelName", StatCollector.translateToLocal(name), showLabels));
        }
        if ((settings & 2) > 0) result.add(new PanelString("msg.ec.InfoPanelAmount", amount, "mB", showLabels));
        if ((settings & 4) > 0)
            result.add(new PanelString("msg.ec.InfoPanelFree", (double) capacity - amount, "mB", showLabels));
        if ((settings & 8) > 0) result.add(new PanelString("msg.ec.InfoPanelCapacity", capacity, "mB", showLabels));
        if ((settings & 16) > 0) result.add(
            new PanelString("msg.ec.InfoPanelPercentage", capacity == 0 ? 100 : (amount * 100 / capacity), showLabels));
        return result;
    }
    ...
    public void renderBars(TextureManager manager, double displayWidth, double displayHeight, ICardReader reader) {
        ...
        tessellator.addVertexWithUV(
            ...
            z,
            textureX + 0,
            textureY + sprite.getMaxV() - sprite.getMinV());
        tessellator.addVertexWithUV(
            x + 0.875,
            y + 0.4375 / 2 + height,
            z,
            textureX + sprite.getMaxU() - sprite.getMinU(),
            textureY + sprite.getMaxV() - sprite.getMinV());
        tessellator.addVertexWithUV(
            x + 0.875,
            y + 0.4375 / 2,
        ...
        IHasBars
            .drawTransparentRect(x + 0.875 - width, y + height + 0.4375 / 2, x, y + 0.4375 / 2, -0.0001, 0xB0000000);
        GL11.glScaled(0.875D / displayWidth, 0.875D / displayHeight, 1);
    }

    public void tryRenderHBMFluid(TextureManager manager, double displayWidth, double displayHeight, ICardReader reader,
        String fluidName) {
        ResourceLocation texture = CrossModLoader.getCrossMod(ModIDs.HBM)
            .getFluidTexture(fluidName);
        ...
        tessellator.addVertexWithUV(
            ...
            y + 0.4375 / 2 + height,
            z,
            4 * (displayWidth + 0.125D),
            2 * (displayHeight + 0.125D));
