...
public class NetworkHelper {
    ...
    public static <REQ extends IMessage, REPLY extends IMessage> void registerClientToServer(
        Class<? extends IMessageHandler<REQ, REPLY>> handler, Class<REQ> request, int id) {
        network.registerMessage(handler, request, id, Side.SERVER);
    }

    public static <REQ extends IMessage, REPLY extends IMessage> void registerServerToClient(
        Class<? extends IMessageHandler<REQ, REPLY>> handler, Class<REQ> request, int id) {
    ...
    public static void sendPacketToAllAround(World world, int x, int y, int z, int dist, IMessage packet) {
        ...
        for (EntityPlayer player : players) if (player instanceof EntityPlayerMP) {
            double dx = x - player.posX;
            double dy = y - player.posY;
            double dz = z - player.posZ;

            if (dx * dx + dy * dy + dz * dz < dist * dist) network.sendTo(packet, (EntityPlayerMP) player);
        }
    }
