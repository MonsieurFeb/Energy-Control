...
public class Screen {
    ...
    public TileEntityInfoPanel getCore(IBlockAccess world) {
        ...
        if (!(te instanceof TileEntityInfoPanel)) return null;
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (te == null || !(te instanceof IScreenPart)) continue;
                    ((IScreenPart) te).setScreen(this);
                    if (powered || force) ((IScreenPart) te).updateTileEntity();
                }
            }
        }
    }
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (powered || force) part.updateTileEntity();
                }
            }
        }
    }
    ...
    private void markUpdate(World world) {
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    TileEntity te = world.getTileEntity(x, y, z);
                    if (te instanceof IScreenPart) ((IScreenPart) te).updateTileEntity();
                }
            }
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (coreX != other.coreX) return false;
        if (coreY != other.coreY) return false;
        if (coreZ != other.coreZ) return false;
