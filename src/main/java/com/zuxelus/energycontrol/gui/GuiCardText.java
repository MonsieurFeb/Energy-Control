...
public class GuiCardText extends GuiBase {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < lineCount; i++) data[i] = reader.getString("line_" + i);
    }

    @Override
    public void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        super.drawGuiContainerBackgroundLayer(partialTicks, mouseX, mouseY);
        if (textArea != null) textArea.drawTextBox();
    }

    @Override
    public void updateScreen() {
        super.updateScreen();
        if (textArea != null) textArea.updateCursorCounter();
    }
    ...
    private void updateTextArea() {
        if (textArea != null) {
            ...
            if (lines != null) for (int i = 0; i < lines.length; i++) reader.setString("line_" + i, lines[i]);
        }
    ...
    protected void keyTyped(char typedChar, int keyCode) {
        if (keyCode == 1 || (keyCode == this.mc.gameSettings.keyBindInventory.getKeyCode()
            && (textArea == null || !textArea.isFocused()))) updateTextArea();
        else if (textArea != null && textArea.isFocused()) textArea.textAreaKeyTyped(typedChar, keyCode);
