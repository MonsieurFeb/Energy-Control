...
public class GregTechHooks {
    ...
    public static void onTick2(MultiTileEntityBoilerTank te, long aTimer, boolean aIsServerSide) {
        ...
        long tConversions = Math
            .min(info[1].capacity / 2560L, Math.min(mEnergy / 80L, info[0].fluid != null ? info[0].fluid.amount : 0));
        ArrayList<Long> values = map.get(te);
        if (values != null && values.size() > 0) {
            for (int i = 19; i > 0; i--) values.set(i, values.get(i - 1));
            values.set(0, tConversions); // water
            for (int i = 39; i > 20; i--) values.set(i, values.get(i - 1));
            values.set(20, Code.units(tConversions, 10000L, mEfficiency * 160, false));
            for (int i = 59; i > 40; i--) values.set(i, values.get(i - 1));
            ...
            for (int i = 0; i < 60; i++) values.add(0L);
            map.put(te, values);
        }
    }
    ...
    public static void doConversion(MultiTileEntityTurbineSteam te, long aTimer) {
        if (!map.containsKey(te) || te.getWorldObj().isRemote) return;
        if (!buffer.containsKey(te)) buffer.put(te, new long[buf_len]);
    ...
    public static void onTick2(TileEntityBase10EnergyConverter te, long aTimer, boolean aIsServerSide) {
        if (!map.containsKey(te) || !buffer.containsKey(te) || !aIsServerSide) return;
        ArrayList<Long> values = map.get(te);
        long[] list = buffer.get(te);
        if (values != null && values.size() > 0) {
        ...
        } else map.put(te, initArray(len * (te instanceof MultiTileEntityTurbineSteam ? 3 : 2)));
    }

    @Hook
    public static void onTick2(TileEntityBase10ConnectorRendered te, long aTimer, boolean aIsServerSide) {
        if (!map.containsKey(te) || !buffer.containsKey(te)
            || !aIsServerSide
            || te instanceof MultiTileEntityWireElectric) return;
        ArrayList<Long> values = map.get(te);
        long[] list = buffer.get(te);
        if (values != null && values.size() > 0) {
            addValue(values, 0, len, list[0]);
            ...
            list[0] = 0;
            list[1] = 0;
        } else map.put(te, initArray(len * 2));
    }
    ...
    public static final long insertEnergyInto(Util te, TagData aEnergyType, byte aSideInto, long aSize, long aAmount,
        Object aEmitter, TileEntity aReceiver, @ReturnValue long rUsedAmount) {
        if (map.containsKey(aReceiver)) {
            if (!buffer.containsKey(aReceiver)) buffer.put(aReceiver, new long[buf_len]);
            long[] list = buffer.get(aReceiver);
            list[0] = list[0] + rUsedAmount * aSize;
            if (list[2] < aSize) list[2] = aSize;
        }
        if (aEmitter instanceof TileEntity && map.containsKey(aEmitter)) {
            if (!buffer.containsKey(aEmitter)) buffer.put((TileEntity) aEmitter, new long[buf_len]);
    ...
    @Hook(at = @At(point = InjectionPoint.RETURN), returnCondition = ReturnCondition.ALWAYS)
    public static int fill(TileEntityBase01Root te, ForgeDirection aDirection, FluidStack aFluid, boolean aDoFill,
        @ReturnValue int rFilledAmount) {
        if (!map.containsKey(te) || !aDoFill) return rFilledAmount;
        if (!buffer.containsKey(te)) buffer.put(te, new long[buf_len]);
        long[] list = buffer.get(te);
    ...
    public static void onTick2(MultiTileEntityWireElectric te, long aTimer, boolean aIsServerSide) {
        if (!map.containsKey(te) || !aIsServerSide) return;
        ArrayList<Long> values = map.get(te);
        // long[] list = buffer.get(te);
        if (values != null && values.size() > 0) {
    ...
    @Hook(
        at = @At(point = InjectionPoint.RETURN),
        targetMethod = "onServerTickPost",
