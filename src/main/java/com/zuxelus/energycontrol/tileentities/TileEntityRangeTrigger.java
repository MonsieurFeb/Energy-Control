...
public class TileEntityRangeTrigger extends TileEntityInventory implements ISlotItemFilter, ITilePacketHandler {
    ...
    public void setLevelStart(long start) {
        if (!worldObj.isRemote && levelStart != start) notifyBlockUpdate();
        levelStart = start;
    }

    public void setLevelEnd(long end) {
        if (!worldObj.isRemote && levelEnd != end) notifyBlockUpdate();
        levelEnd = end;
    }
    ...
    public void onServerMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                if (tag.hasKey("value")) setLevelStart(tag.getLong("value"));
                break;
            case 2:
                if (tag.hasKey("value")) setInvertRedstone(tag.getInteger("value") == 1);
                break;
            case 3:
                if (tag.hasKey("value")) setLevelEnd(tag.getLong("value"));
                break;
        }
    }
    ...
    public void markDirty() {
        ...
        if (ItemCardMain.isCard(card)) {
            ...
            CardState state = ItemCardMain
                .updateCardNBT(card, worldObj, xCoord, yCoord, zCoord, reader, getStackInSlot(SLOT_UPGRADE));
            if (state == CardState.OK) {
                double cur = 0;
                if (reader.hasField(DataHelper.ENERGY)) cur = reader.getDouble(DataHelper.ENERGY);
                else if (reader.hasField(DataHelper.AMOUNT)) cur = reader.getDouble(DataHelper.AMOUNT);
                status = cur > Math.max(levelStart, levelEnd) || cur < Math.min(levelStart, levelEnd) ? STATE_ACTIVE
                    : STATE_PASSIVE;
            } else status = STATE_UNKNOWN;
        }
        setStatus(status);
    }
    ...
    public void notifyBlockUpdate() {
        Block block = worldObj.getBlock(xCoord, yCoord, zCoord);
        if (!(block instanceof RangeTrigger)) return;
    ...
    public boolean isItemValid(int slotIndex, ItemStack stack) { // ISlotItemFilter
        if (slotIndex == SLOT_CARD) return ItemCardMain.isCard(stack);
