...
public abstract class HookInjectorMethodVisitor extends AdviceAdapter {
    ...
    public static class ByAnchor extends HookInjectorMethodVisitor {
        ...
        public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
            String targetName = HookLibPlugin.getObfuscated() ? MinecraftClassTransformer.instance.getMethodNames()
                .getOrDefault(MinecraftClassTransformer.getMethodId(name), name) : name;
            if (hook.getAnchorPoint() == METHOD_CALL && hook.getAnchorTarget()
                .equals(targetName)) switch (hook.getShift()) {
                ...
                case INSTEAD:
                    if (visitOrderedHook())
                        for (int i = 0; i < Type.getArgumentTypes(desc).length + 1; i++) visitInsn(Opcodes.POP);
        ...
        protected void onMethodEnter() {
            if (hook.getAnchorPoint() == HEAD) visitHook();
        }

        protected void onMethodExit(int opcode) {
            if (hook.getAnchorPoint() == InjectionPoint.RETURN && opcode != Opcodes.ATHROW) visitOrderedHook();
