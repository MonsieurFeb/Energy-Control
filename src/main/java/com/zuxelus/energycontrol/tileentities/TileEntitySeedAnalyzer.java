...
public class TileEntitySeedAnalyzer extends TileEntityInventory
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("productionMax")) productionMax = tag.getInteger("productionMax");
        if (tag.hasKey("active")) {
            boolean old = active;
            active = tag.getBoolean("active");
            if (worldObj.isRemote && active != old) worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        }
    }
    ...
    private void updateActive() {
        active = false;
        productionMax = 0;
        if (energy < CONSUMPTION) return;
        ItemStack out = getStackInSlot(SLOT_OUT);
        if (out != null) return;
        ItemStack stack = getStackInSlot(SLOT_IN);
        if (stack == null || stack.getItem() != Ic2Items.cropSeed.getItem()) return;
    ...
    public boolean canInsertItem(int slot, ItemStack stack, int side) {
        if (slot != SLOT_IN || side != 1) return false;
        return stack.getItem() == Ic2Items.cropSeed.getItem();
    }

    @Override
    public boolean canExtractItem(int slot, ItemStack stack, int side) {
        if (slot != SLOT_OUT || side == 1) return false;
        ItemStack crop = getStackInSlot(slot);
        if (crop == null || crop.getItem() != Ic2Items.cropSeed.getItem()) return false;
        if (side == 0) return ItemCropSeed.getScannedFromStack(crop) == 4;
        if (side != 0) return ItemCropSeed.getScannedFromStack(crop) < 4;
        return true;
    }
    ...
    public double injectEnergy(ForgeDirection directionFrom, double amount, double voltage) {
        ...
        if (energy >= CONSUMPTION && old == 0 && !worldObj.isRemote) updateState();
        return left;
    }
