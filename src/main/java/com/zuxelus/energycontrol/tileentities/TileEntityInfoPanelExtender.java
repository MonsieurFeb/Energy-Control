...
public class TileEntityInfoPanelExtender extends TileEntityFacing implements IScreenPart {
    ...
    private void updateScreen() {
        ...
        if (worldObj.isRemote && !partOfScreen && screen != null) setScreen(null);
    }
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("partOfScreen")) partOfScreen = tag.getBoolean("partOfScreen");
        if (tag.hasKey("coreX")) {
        ...
        if (worldObj != null) {
            updateScreen();
            if (worldObj.isRemote) worldObj.func_147451_t(xCoord, yCoord, zCoord);
        }
    }
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote && !partOfScreen) EnergyControl.instance.screenManager.registerInfoPanelExtender(this);
    ...
    public void setScreen(Screen screen) {
        ...
        if (screen != null) {
            ...
            if (core != null) {
                ...
                if (meta != metaCore) worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, metaCore, 2);
                return;
            }
        } else {
            int meta = worldObj.getBlockMetadata(xCoord, yCoord, zCoord);
            if (meta > 5) worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, meta - 6, 2);
        }
        partOfScreen = false;
        coreX = 0;
        coreY = 0;
