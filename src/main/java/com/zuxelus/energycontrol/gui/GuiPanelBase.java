...
public abstract class GuiPanelBase extends GuiContainerBase implements ICrafting {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (textboxTitle != null) {
            ...
            if (textboxTitle.isFocused() != focused) updateTitle();
        }
    }
    ...
    public void updateScreen() {
        super.updateScreen();
        if (textboxTitle != null) textboxTitle.updateCursorCounter();
    }
    ...
    protected void updateTitle() {
        ...
        if (panel.getWorldObj().isRemote) {
            ...
            if (ItemCardMain.isCard(card)) new ItemCardReader(card).setTitle(textboxTitle.getText());
        }
    }
    ...
    protected void keyTyped(char typedChar, int keyCode) {
        if (textboxTitle != null && textboxTitle.isFocused()) if (keyCode == 1) mc.thePlayer.closeScreen();
        else if (typedChar == 13) updateTitle();
        else {
            modified = true;
