...
public class TileEntityHowlerAlarm extends TileEntityFacing implements ITilePacketHandler {
    ...
    public void setRange(int r) {
        if (!worldObj.isRemote && range != r) notifyBlockUpdate();
        range = r;
    }
    ...
    public void setSoundName(String name) {
        soundName = name;
        if (!worldObj.isRemote && !prevSoundName.equals(soundName)) notifyBlockUpdate();
        if (worldObj.isRemote) {
            if (EnergyControl.instance.availableAlarms != null
                && !EnergyControl.instance.availableAlarms.contains(soundName)) {
                EnergyControl.logger.info(
                    String
                        .format("Can't set sound '%s' at %d,%d,%d, using default", soundName, xCoord, yCoord, zCoord));
    ...
    public void onServerMessageReceived(NBTTagCompound tag) {
        ...
        switch (tag.getInteger("type")) {
            case 1:
                if (tag.hasKey("string")) setSoundName(tag.getString("string"));
                break;
            case 2:
                if (tag.hasKey("value")) setRange(tag.getInteger("value"));
                break;
        }
    }
    ...
    protected void readProperties(NBTTagCompound tag) {
        ...
        if (tag.hasKey("soundName")) soundName = prevSoundName = tag.getString("soundName");
        if (tag.hasKey("range")) range = tag.getInteger("range");
        if (tag.hasKey("powered")) updatePowered(tag.getBoolean("powered"));
    }
    ...
    public void invalidate() {
        if (worldObj.isRemote && sound != null) sound.stopAlarm();
        super.invalidate();
    }

    @Override
    public void updateEntity() {
        if (worldObj.isRemote) checkStatus();
    }

    protected void checkStatus() {
        if (sound == null) sound = new TileEntitySound();
        if (!sound.isPlaying()) updateTicker--;
