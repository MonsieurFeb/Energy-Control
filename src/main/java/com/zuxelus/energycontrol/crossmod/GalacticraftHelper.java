...
public class GalacticraftHelper {
    ...
    public static String getStatus(TileEntityOxygenCollector collector) {
        ...
        if (returnValue.equals(EnumColor.DARK_GREEN + GCCoreUtil.translate("gui.status.active.name"))
            && collector.lastOxygenCollected <= 0.0F)
    ...
    public static String getThermalStatus(TileEntityOxygenSealer sealer) {
        Block blockAbove = sealer.getWorldObj()
            .getBlock(sealer.xCoord, sealer.yCoord + 1, sealer.zCoord);
        int metadata = sealer.getWorldObj()
            .getBlockMetadata(sealer.xCoord, sealer.yCoord + 1, sealer.zCoord);
        if (blockAbove == GCBlocks.breatheableAir || blockAbove == GCBlocks.brightBreatheableAir)
            if (metadata == 1) return EnumColor.DARK_GREEN + GCCoreUtil.translate("gui.status.on.name");
    ...
    public static String getStatus(TileEntityOxygenSealer sealer) {
        ...
        if (blockAbove != null
            ...
            && !OxygenPressureProtocol.canBlockPassAir(
                sealer.getWorldObj(),
                blockAbove,
                new BlockVec3(sealer.xCoord, sealer.yCoord + 1, sealer.zCoord),
                1))
            return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.sealerblocked.name");

        if (sealer.disabled) return EnumColor.ORANGE + GCCoreUtil.translate("gui.status.disabled.name");
        ...
        if (sealer.storedOxygen < 1) return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.missingoxygen.name");
        ...
        if (threadCooldown < 15) {
            if (threadCooldown < 4) {
                String elipsis = "";
                for (int i = 0; i < (23 - threadCooldown) % 4; i++) elipsis += ".";
        ...
        if (!sealer.sealed) return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.unsealed.name");
    ...
    public static String getStatus(TileEntityRefinery te) {
        ...
        else displayText = EnumColor.RED + GCCoreUtil.translate("gui.status.unknown.name");
        return displayText;
    }
    ...
    public static String getStatus(TileEntityMethaneSynthesizer te) {
        ...
        else displayText = EnumColor.RED + GCCoreUtil.translate("gui.status.needsCarbon.name");
        return displayText;
    }
    ...
    public static String getStatus(TileEntitySolar te) {
        if (te.getDisabled(0)) return EnumColor.ORANGE + GCCoreUtil.translate("gui.status.disabled.name");
        if (!te.getWorld()
            .isDaytime()) return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.blockedfully.name");
        if (te.getWorld()
            .isRaining()
            || te.getWorld()
                .isThundering())
            return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.raining.name");
        if (te.solarStrength == 0) return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.blockedfully.name");
        if (te.solarStrength < 9) return EnumColor.DARK_RED + GCCoreUtil.translate("gui.status.blockedpartial.name");
    ...
    public static String getStatus(TileEntityLaunchController te) {
        if (!te.frequencyValid) return EnumColor.RED + GCCoreUtil.translate("gui.message.invalidFreq.name");
        if (te.getEnergyStoredGC() <= 0.0F) return EnumColor.RED + GCCoreUtil.translate("gui.message.noEnergy.name");
        if (te.getDisabled(0)) return EnumColor.ORANGE + GCCoreUtil.translate("gui.status.disabled.name");
