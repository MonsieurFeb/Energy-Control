...
public class ItemAFSUUpgradeKit extends Item {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (!(block instanceof BlockElectric)) return false;
        TileEntity te = world.getTileEntity(x, y, z);
        if (!(te instanceof TileEntityElectricMFSU)) return false;
        ...
        for (int i = 0; i < items.length; i++) items[i] = ((TileEntityElectricBlock) mfsu).getStackInSlot(i);
        ...
        for (int j = 0; j < items.length; j++) afsu.setInventorySlotContents(j, items[j]);
