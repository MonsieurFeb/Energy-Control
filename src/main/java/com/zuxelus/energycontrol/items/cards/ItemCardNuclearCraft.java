...
public class ItemCardNuclearCraft extends ItemCardBase {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        NBTTagCompound tag = CrossModLoader.getCrossMod(ModIDs.NUCLEAR_CRAFT)
            .getCardData(te);
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        if (reader.hasField(DataHelper.CAPACITY) && (settings & 2) > 0) result
            .add(new PanelString("msg.ec.InfoPanelCapacity", reader.getLong(DataHelper.CAPACITY), "RF", showLabels));
        if (reader.hasField(DataHelper.DIFF) && (settings & 64) > 0) result
            .add(new PanelString("msg.ec.InfoPanelDifference", reader.getLong(DataHelper.DIFF), "RF/t", showLabels));
        ...
        if (reader.hasField(DataHelper.OUTPUT) && (settings & 4) > 0) result
            .add(new PanelString("msg.ec.InfoPanelOutput", reader.getDouble(DataHelper.OUTPUT), "RF/t", showLabels));
        ...
        if (reader.hasField("level1")) result.add(new PanelString(reader.getString("level1")));
        if (reader.hasField("level2")) result.add(new PanelString(reader.getString("level2")));
         ...
         * result.add(new PanelString(I18n.format(reader.getString("gasTankName")) + ": " +
         * reader.getString("gasTank")));
         ...
         * result.add(new PanelString(I18n.format(reader.getString("liquidTankName")) + ": " +
         * reader.getString("liquidTank")));
         * if (!reader.getString("liquidTank2Name").equals(""))
         * result.add(new PanelString(I18n.format(reader.getString("liquidTank2Name")) + ": " +
         * reader.getString("liquidTank2")));
         * break;
         * case 10:
