...
public class TERemoteThermalMonitorRenderer extends TileEntitySpecialRenderer {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
        EnergyControl.MODID,
        "textures/blocks/remote_thermo/all.png");
    ...
    public void renderTileEntityAt(TileEntityRemoteThermalMonitor te, double x, double y, double z) {
        ...
        if (status > -2) {
            double rate = 1;
            if (status > -1) rate = Math.round((1 - Math.min((double) heat / level, 1)) * 16) / (double) 16;
        ...
        func_147498_b()
            .drawString(text, -func_147498_b().getStringWidth(text) / 2, -func_147498_b().FONT_HEIGHT, 0x000000);
        GL11.glPopMatrix();
    }
