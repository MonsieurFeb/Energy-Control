...
public class SeedLibrarySort implements java.util.Comparator<ItemStack> {
    ...
    public static SeedLibrarySort getSort(int type, boolean desc) {
        if (type == NO_SORT) return NONE;

        if (type == GROWTH) {
            if (desc) return GROWTH_DESC;
            return GROWTH_ASC;
        } else if (type == GAIN) {
            if (desc) return GAIN_DESC;
            return GAIN_ASC;
        } else if (type == RESISTANCE) {
            if (desc) return RESISTANCE_DESC;
            return RESISTANCE_ASC;
        } else { // if (type == TOTAL) {
            if (desc) return TOTAL_DESC;
            return TOTAL_ASC;
        }
    }
    ...
    public int compare(ItemStack seed_1, ItemStack seed_2) {
        ...
        if (scan_1 != scan_2) return scan_1 - scan_2;
        if (scan_1 < 4) return 0;
        ...
        if (sort_type == GROWTH) {
            ...
            value_1 = ItemCropSeed.getGrowthFromStack(seed_1) + ItemCropSeed.getGainFromStack(seed_1)
                + ItemCropSeed.getResistanceFromStack(seed_1);
            value_2 = ItemCropSeed.getGrowthFromStack(seed_2) + ItemCropSeed.getGainFromStack(seed_2)
                + ItemCropSeed.getResistanceFromStack(seed_2);
        }

        if (descending) return value_1 - value_2;
