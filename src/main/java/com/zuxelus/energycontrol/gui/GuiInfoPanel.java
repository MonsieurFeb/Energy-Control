...
public class GuiInfoPanel extends GuiPanelBase {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
        EnergyControl.MODID,
        "textures/gui/gui_info_panel.png");
    ...
    protected void initButtons() {
        ...
        if (isColored) buttonList.add(
            new GuiButtonGeneral(ID_COLORS, guiLeft + xSize - 24, guiTop + 42 + 17, 16, 16, TEXTURE, 192, 0)
                .setGradient()
                .setScale(2));
        buttonList.add(
            new GuiButtonGeneral(
    ...
    protected void initControls() {
        ItemStack stack = panel.getCards()
            .get(activeTab);
        if (ItemStack.areItemStacksEqual(stack, oldStack) && panel.isColoredEval() == isColored) return;
        ...
        if (ItemCardMain.isCard(stack)) {
            ...
            if (stack.getItemDamage() == ItemCardType.CARD_TEXT) buttonList.add(
                new GuiButtonGeneral(ID_TEXT, guiLeft + xSize - 24, guiTop + 42 + 17 * 2, 16, 16, "txt").setGradient());
