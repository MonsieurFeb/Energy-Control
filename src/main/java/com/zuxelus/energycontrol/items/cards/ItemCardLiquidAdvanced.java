...
public class ItemCardLiquidAdvanced extends ItemCardBase {
    ...
    public CardState update(World world, ICardReader reader, int range, int x, int y, int z) {
        ...
        if (tanks == null || tanks.size() < 1) return CardState.NO_TARGET;
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
        ...
        for (int i = 0; i < count; i++) addTankData(result, settings, reader, isServer, showLabels, i);
        return result;
    }

    private void addTankData(List<PanelString> result, int settings, ICardReader reader, boolean isServer,
        boolean showLabels, int i) {
        if (!reader.hasField(String.format("_%dcapacity", i))) return;
