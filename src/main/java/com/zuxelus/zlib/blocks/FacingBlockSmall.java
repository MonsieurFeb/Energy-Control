...
public abstract class FacingBlockSmall extends FacingBlock {
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        for (ForgeDirection enumfacing : ForgeDirection.VALID_DIRECTIONS)
            if (canPlaceBlock(world, x, y, z, enumfacing)) return true;
        return false;
    }

    protected static boolean canPlaceBlock(World world, int x, int y, int z, ForgeDirection direction) {
        return world
            .isSideSolid(x + direction.offsetX, y + direction.offsetY, z + direction.offsetZ, direction.getOpposite());
    }
    ...
    public int getMetaForPlacement(World world, int x, int y, int z, int facing, float hitX, float hitY, float hitZ,
        ...
        return canPlaceBlock(
            ...
            ForgeDirection.getOrientation(facing)
                .getOpposite()) ? facing : ForgeDirection.DOWN.ordinal();
    }

    @Override
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        int meta = world.getBlockMetadata(x, y, z);
        if (checkForDrop(world, x, y, z, meta) && !canPlaceBlock(
            world,
            x,
            y,
            z,
