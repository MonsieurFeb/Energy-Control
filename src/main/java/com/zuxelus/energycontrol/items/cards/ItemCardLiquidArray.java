...
public class ItemCardLiquidArray extends ItemCardBase {
        ...
        for (int i = 0; i < cardCount; i++) {
            ...
            if (Math.abs(dx) <= range && Math.abs(dy) <= range && Math.abs(dz) <= range) {
                ...
                if (storage != null) {
                    ...
                    foundAny = true;
                } else reader.setLong(String.format("_%damount", i), STATUS_NOT_FOUND);
            } else {
    ...
    public List<PanelString> getStringData(int settings, ICardReader reader, boolean isServer, boolean showLabels) {
                ...
                if (isOutOfRange) {
                        ...
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelLiquidNameN",
                                    i + 1,
                                    reader.getString(String.format("_%dname", i)))));
                        else result.add(new PanelString(StringUtils.getFormatted("", amount, false)));
                    }
                    if (showAmount) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelLiquidN",
                                    i + 1,
                    ...
                    if (showFree) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelLiquidFreeN",
                                    i + 1,
                                    StringUtils.getFormatted("", capacity - amount, false))));
                        else result.add(new PanelString(StringUtils.getFormatted("", capacity - amount, false)));
                    }
                    if (showCapacity) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelLiquidCapacityN",
                                    i + 1,
                                    StringUtils.getFormatted("", capacity, false))));
                        else result.add(new PanelString(StringUtils.getFormatted("", capacity, false)));
                    }
                    if (showPercentage) {
                        if (showLabels) result.add(
                            new PanelString(
                                StringUtils.getFormattedKey(
                                    "msg.ec.InfoPanelLiquidPercentageN",
                                    i + 1,
        ...
        if (showSummary) {
            if (showAmount) result.add(new PanelString("msg.ec.InfoPanelAmount", totalAmount, showLabels));
            if (showFree) result.add(new PanelString("msg.ec.InfoPanelFree", totalCapacity - totalAmount, showLabels));
            if (showName) result.add(new PanelString("msg.ec.InfoPanelCapacity", totalCapacity, showLabels));
